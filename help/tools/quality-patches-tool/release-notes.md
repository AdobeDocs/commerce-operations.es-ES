...
* **ACSD-49392** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.0) || >= 2.4.1 &lt;2.4.7): corrige el problema en el que el estado de pedido cambia a cerrado después de un reembolso parcial para un producto agrupado.
* **ACSD-51036** (para Adobe Commerce y Magento Open Source >=2.4.4 &lt;2.4.5) : corrige el problema en el que las condiciones de carrera durante las llamadas de API REST simultáneas resultan en una sobrescritura de la información del estado de envío en la variable [!UICONTROL Items Ordered] tabla.
* **ACSD-50858** (para Adobe Commerce y Magento Open Source >=2.4.4 &lt;2.4.7): mejora el rendimiento para cargar el contenido de los banners.
* Se han añadido nuevas versiones para MDVA-39305-v2, ACSD-45169.
* Revisiones ACSD-50260-v2 actualizadas.

## v1.1.30 {#v1-1-30}

* **ACSD-50336** (para Adobe Commerce y Magento Open Source >=2.4.4-p1 &lt;2.4.4-p3) : corrige el problema en el que los correos electrónicos de alerta de producto no se envían cuando un producto vuelve a estar en existencias o se cambia el precio.
* **ACSD-50367** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que la exportación de direcciones de clientes no funciona cuando se crea un atributo de direcciones de clientes de selección múltiple sin valores.
* **ACSD-49877** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que la reproducción automática de vídeo no funciona en dispositivos móviles [!DNL Safari] cuando el vídeo está vinculado directamente a un archivo de vídeo remoto y no a un servicio de flujo continuo.
* **ACSD-50165** (para Adobe Commerce y Magento Open Source >=2.4.0 &lt;2.4.7): corrige el error *El archivo no se puede eliminar. Warning!unlink: No existe dicho archivo o directorio* al vaciar la caché de JS/CSS desde el administrador.
* **ACSD-49737** (para Adobe Commerce y Magento Open Source >=2.4.1-p1 &lt;2.4.7) : corrige el problema en el que un cupón se marca incorrectamente como utilizado tras un pago con tarjeta fallido.
...
* **ACSD-49129** (para Adobe Commerce y Magento Open Source >=2.4.2 &lt;2.4.6): corrige el problema en el que la variable *[!UICONTROL content]* El atributo (código de imagen base64) no se devuelve en `rest/V1/products/sku/media` respuestas de API de medios de producto.
* **ACSD-50276** (para Adobe Commerce >=2.4.0 &lt;2.4.7) : corrige el problema en el que el formulario de registro de clientes no funciona en la tienda si se crea un atributo de cliente de selección múltiple.
...
* **ACSD-49389** (para Adobe Commerce y Magento Open Source >=2.4.0 &lt;2.4.7) : corrige el problema en el que API envía un correo electrónico listo para recibir cuando el pedido no está listo para recibir.
* **ACSD-49822** (para Adobe Commerce >=2.3.7 &lt;2.4.7): corrige el problema en el que las actualizaciones en la variable [!UICONTROL Requisition List] no se reflejan en la [!UICONTROL Print Requisition List].
* **ACSD-48771** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.7) : corrige el problema de actualizar el tipo de contenido de bloque de columna de anterior [!DNL Page Builder] versiones.
* **ACSD-49464** (para Adobe Commerce >=2.3.7 &lt;2.4.7) : corrige el problema en el que las facturas, los envíos y las notas de crédito no se mueven de nuevo del archivo cuando el orderId es diferente.
...
* **ACSD-49527** (para Adobe Commerce >=2.4.2 &lt;2.4.7) : corrige el problema en el que las funciones de empresa de GraphQL no muestran la paginación correctamente.
* **ACSD-49706** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que el valor predeterminado se guarda para un atributo de muestra visual cuando no se selecciona ningún valor.
* **ACSD-49835** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.7) : corrige el problema en el que el valor de la casilla de verificación Usar predeterminado no se guardaba correctamente en un nivel de tienda para un atributo de selección múltiple.
* **ACSD-49898** (para Adobe Commerce y Magento Open Source >=2.4.4 &lt;2.4.6) : corrige el problema en el que la cuadrícula de productos genera una excepción cuando un producto empaquetado tiene un precio especial que supera los 1000.
...
* **ACSD-49849** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.6) : corrige el problema por el que se reemplazaba el correo electrónico del cliente con [!DNL PayPal] correo electrónico al colocar un pedido con [!DNL PayPal Express] a través de GraphQL.
* **ACSD-49839** (para Adobe Commerce >=2.3.7 &lt;2.4.7) : corrige el problema en el que los precios y la estructura del catálogo compartido genera un error en Administración cuando los productos tienen comillas simples o dobles en SKU.
* **ACSD-49970** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.7) : corrige la administración incorrecta de los errores de GraphQL cuando [!DNL New Relic] el sistema de informes está activado.
* **ACSD-50260** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.7) : corrige el problema en el que los resultados de búsqueda de productos de GraphQL están limitados a 10.000 resultados únicamente.
...
* **ACSD-48204** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.3) : corrige el problema en el que una regla de precio de catálogo creada basándose en el atributo Sí/No no tiene en cuenta el ámbito seleccionado.
* **ACSD-47704** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que el producto empaquetado muestra el precio de solo en los productos bursátiles.
* **ACSD-49370** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7): corrige el problema en el que la variable *Fecha y hora* el atributo de producto tiene un *FilterMatchTypeInput* en esquema de GraphQL.
* **ACSD-48807** (para Adobe Commerce y Magento Open Source >=2.4.1 &lt;2.4.7) : corrige el problema en el que las revisiones de productos de los clientes no se filtran mediante una vista de almacén a través de GraphQL.
* **ACSD-49433** (para Adobe Commerce y Magento Open Source >=2.4.3 &lt;2.4.7) : corrige el problema en el que la cantidad predeterminada se muestra como subtotal en el carro de la tarjeta de regalo con una cantidad abierta.
* **ACSD-48866** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que se producía un error al solicitar la fuente RSS para las categorías.
* **ACSD-48784** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que los precios de los segmentos de clientes se almacenan en caché incorrectamente entre grupos de clientes.
* **ACSD-48857** (para Adobe Commerce y Magento Open Source >=2.4.3 &lt;2.4.7) : corrige el problema en el que un usuario no puede guardar los cambios después de editarlos con Page Builder.
* **ACSD-49065** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que los elementos de presupuesto no son visibles en el Administrador si solo están asignados al inventario de existencias personalizado.
* **ACSD-49179** (para Adobe Commerce y Magento Open Source >=2.4.2 &lt;2.4.7): corrige el problema en el que el informe de pedidos muestra cantidades incorrectas en el caso de monedas diferentes para diferentes tiendas.
* **ACSD-49286** (para Adobe Commerce y Magento Open Source >=2.4.3 &lt;2.4.7) : corrige el problema en el que un producto se añade dos veces al carro de compras cuando hay varias utilidades de producto presentes en la página.
* **ACSD-49574** (para Adobe Commerce >=2.4.4 &lt;2.4.7): Añade funcionalidad para admitir las actualizaciones de producto de la tarjeta de regalo en un carro de compras a través de GraphQL.
* Parche actualizado: ACSD-48694.

## v1.1.27 {#v1-1-27}

* **ACSD-48362** (para Adobe Commerce >=2.4.1 &lt;2.4.7): corrige el problema en el que se utiliza la dirección de envío predeterminada en lugar de una nueva al realizar un pedido con una oferta negociable.
* **ACSD-48059** (para Adobe Commerce >=2.3.7 &lt;2.4.7): corrige el problema en el que los comerciantes no pueden guardar el &quot;[!UICONTROL Match product by rule]&quot; en la categoría .
* **ACSD-48216** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7): corrige el problema en el que [!UICONTROL AUTO_INCREMENT] del [!UICONTROL inventory_source_item] la tabla aumenta en el [!UICONTROL UPDATE] operación.
...
* **ACSD-49497** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.6) : corrige el problema en el que un pedido permanece en estado de procesamiento después del envío y se aplica un reembolso parcial.
* **ACSD-48694** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.1 &lt;2.4.7): corrige el problema en el que el error &quot;Cambio de estado no válido solicitado&quot; impide que un cliente realice un pedido.
* **ACSD-49013** (para Adobe Commerce y Magento Open Source >=2.4.3 &lt;2.4.7) : corrige el problema en el que la confirmación de correo electrónico no se traduce a la configuración regional del sitio web al crear clientes mediante API masiva.
* **ACSD-48164** (para Adobe Commerce y Magento Open Source >=2.3.7 &lt;2.4.7) : corrige el problema en el que un administrador restringido no puede guardar un valor en el nivel de sitio web.
...
* **ACSD-49042** (para Adobe Commerce y Magento Open Source >=2.4.4 &lt;2.4.5) : corrige el problema en el que un producto con un orden de servidor infinito no se puede solicitar desde la tienda.
...
* **ACSD-48262** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.6) : corrige el problema en el que los productos no están visibles en el front-end cuando la opción &quot;Permitir todos los productos por página&quot; está establecida en Sí.
* **ACSD-48293** (para Adobe Commerce y Magento Open Source >=2.4.3 &lt;2.4.4): corrige el problema en el que los productos compuestos se quedan sin existencias cuando los productos secundarios que se agotaron se devuelven a la existencias.
...
* **ACSD-48044** (para Adobe Commerce y Magento Open Source >=2.4.0 &lt;2.4.4) : corrige el problema en el que la aplicación de varias tarjetas de regalo a un único pedido con varios envíos impide que se realicen pedidos.
* **ACSD-48300** (para Adobe Commerce y Magento Open Source >=2.4.0 &lt;2.4.6) : corrige el problema en el que no se puede crear un retorno si se elimina el producto configurable.
* **ACSD-47910** (para Adobe Commerce y Magento Open Source >=2.4.4 &lt;2.4.6): corrige el problema de pedidos, facturas, envíos y notas de abono que faltan en las respectivas cuadrículas de entidades.
* **ACSD-47292** (para Adobe Commerce y Magento Open Source >=2.4.4 &lt;2.4.6): corrige el problema en el que los productos empaquetados fuera de existencias no están disponibles en la respuesta de GraphQL si la opción &quot;mostrar productos sin existencias&quot; está establecida en Sí.
* **ACSD-48234** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.6) : corrige el problema en el que el resultado de la búsqueda en el catálogo muestra un recuento incorrecto de elementos de la categoría cuando la opción &quot;mostrar sin existencias&quot; está activada.
* **ACSD-48313** (para Adobe Commerce y Magento Open Source >=2.4.0 &lt;2.4.5) : corrige el problema en el que la columna &quot;variaciones configurables&quot; no se analiza si el valor del atributo contiene una coma. El mismo algoritmo de análisis se usa para &quot;additional_attributes.
* **ACSD-48627** (para Adobe Commerce y Magento Open Source >=2.4.5 &lt;2.4.6) : corrige el problema en el que el producto configurable fuera de existencias provoca un error al enviar una solicitud de GraphQL para obtener detalles del carro de compras.
* Parche actualizado: MDVA-39384.
...
* **MDVA-42341** (*para Adobe Commerce y Magento Open Source >=2.4.2 &lt;2.4.4*): Corrige el problema en el que la variable `categoryList` La consulta de GraphQL no filtra los resultados si una solicitud tiene el encabezado Store .
...
* **MDVA-39031** (*para Adobe Commerce y Magento Open Source >=2.4.1 &lt;2.4.4*): Corrige el problema en el que es posible añadir un producto al carro de compras a través de GraphQL aunque no esté asignado al sitio web de destino.
...
* **MDVA-38447** (*para Adobe Commerce y Magento Open Source >=2.4.2 &lt;2.4.4*): corrige dos problemas: donde *No visible individualmente* los productos secundarios configurables se devuelven en la respuesta de GraphQL y optimizan la consulta MySQL para la consulta de productos de GraphQL con filtro de categoría.
...
* **MDVA-37779** (*para Adobe Commerce >=2.4.2 &lt;=2.4.4*): Corrige el problema de añadir un producto configurable al carro de compras a través de GraphQL cuando el ID del sitio web no coincide con el ID de la tienda.
...
* **MDVA-35197** (*para Adobe Commerce >=2.3.5 &lt;2.4.0*): Corrige el problema en el que se producía un error al añadir al carro de compras con GraphQL si los productos añadidos anteriormente no estaban en existencias.
...
* **MDVA-33975** (*para Adobe Commerce >=2.3.4 &lt;2.4.2*): Corrige varios problemas relacionados con el cálculo de precios en las solicitudes de GraphQL.
...
* **MDVA-32714** (*para Adobe Commerce >=2.3.4 &lt;2.4.1*): Corrige el problema en el que el precio del grupo de clientes no funciona en la consulta de productos de GraphQL.
