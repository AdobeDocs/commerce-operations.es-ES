...
---
# Problemas corregidos de Adobe Systems Commerce (v2.4.8)

## Problemas corregidos en la versión 2.4.8

Hemos corregido 582 problemas en el código central de Adobe Systems Commerce 2.4.8. A continuación se describe un subconjunto de los problemas corregidos incluidos en esta versión.
...
* _ACP2E-2703_: API de REST que muestra pedidos de otro sitio web.
   * _Nota de corrección_: El sistema ahora admite ámbito acceso autorizado para los tokens de administrador de la API de REST y los puntos finales de Magento_Sales, lo que garantiza que la API de REST solo muestre los pedidos a los que el administrador tiene acceso. Anteriormente, la API de REST mostraba los pedidos de todos los sitios web, independientemente del sitio web asignado del usuario administrador.
...
* _ACP2E-3079_: Error de actualización de dirección del cliente al actualizar a través de la API de REST si &quot;validateDefaultAddress&quot; está habilitado
   * _Nota_ de corrección: El punto final de la API ahora funciona según lo previsto después de que se haya resuelto el problema en el que falta la clave de ID de la carga útil de la API.
...
* _ACP2E-3486_: No se establecen valores predeterminados para los atributos de fecha y hora con la API RestAPI de productos
   * _Nota de corrección_: Los valores predeterminados ahora se establecen correctamente para los atributos de fecha y fecha y hora mediante RestAPI
...
* _ACP2E-3348_: no hay graphQl disponible para suscribirse a las actualizaciones de la Puntos de recompensas para el cliente
   * _Nota_ de corrección: Antes de la corrección, el atributo del cliente reward_warning_notificación no se podía actualizar a través de la mutación GraphQL y la llamada a la API de REST. Ahora se puede actualizar igual que el atributo del cliente reward_update_notificación.

### API, GraphQL, Impuestos

* _AC-12060_: tanto Luma (API de Rest) como Graphql no calculan impuestos cuando solo se proporciona el código postal.
...
* _AC-14299_: El front-end IU página principal en mi menú desplegable perfil está botón no está allí.(intermitentemente)
...
* _ACP2E-3115_: [La nube no puede crear cliente a través de la API cuando la venta] privada está habilitada
   * _Nota_ de corrección: Ahora el cliente se puede crear mediante usuario de administración autenticado, así como con un token de integración autenticado a través de la API de REST cuando la restricción del sitio web está habilitada.
...
* _ACP2E-3433_: La configuración de Permitir países causa problemas en las configuraciones de direcciones de clientes
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/078c387e>
* _ACP2E-3445_: Registro de regalos compartidos muestra la fecha evento como 1 día antes
   * _Nota_ de corrección: La fecha del Registro de regalos se muestra correctamente ahora en Storefront
...
* _AC-13767_: Cuando el número máximo de Contraseña solicitudes de Restablecer&quot; se establece en un valor mayor que 0, p. ej.: 3 , &quot;Los mensajes de error que superan el límite se envían antes de volver a guardar el límite, es decir, desde la segunda vez
* _AC-13768_: Aunque el número máximo de solicitudes de Restablecer de Contraseña&quot; está establecido en 0 (diabled), &quot;Los mensajes de error que superan el límite se envían desde la segunda vez&quot;
* _AC-13850_: No hay asterisco rojo para el campo obligatorio de número de teléfono
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/c699c206>
* _AC-14300_: En administración Cuando intentamos reordenar enviar pedido botón no es se puede hacer clic. (intermitentemente)
...
* _AC-9843_: i18n:collect-phrases rompe la integridad de las traducciones
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/0c53bbf7>
* _ACP2E-2687_: Problema de permiso para acceder al bloque dinámico
   * _Nota_ de corrección: Anteriormente, para el administrador restringido, agregar un nuevo bloque dinámico arrojaba un error. Después de implementar esta corrección, el administrador restringido puede agregar con éxito el bloque dinámico y editarlo sin ningún error
...
* _ACP2E-3373_: administración de Magento URL enrutamiento detección incorrecta y errores CORS
   * _Nota_ de corrección: Después de la corrección, si el dominio de administración personalizado es un subdominio del dominio principal, solo se puede acceder al administrador desde el subdominio configurado.
...
* _ACP2E-3392_: roto validación para &quot;Cantidad máxima permitida en el carro de compras&quot;
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/d50f6b5d>
* _ACP2E-3408_: [Problema IU Vista previa Pagebuilder] Los botones de la columna Page Builder no se alinean correctamente
   * _Nota_ de corrección: Los botones de las columnas Page Builder ahora están alineados correctamente. Anteriormente, estaban desalineadas dentro de las columnas Page Builder.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2-page-builder/commit/1a52ef4c>
...
* _ACP2E-3457_: TinyMCE JS Error en la consola después de la habilitación de js minificación con modo de producción
   * _Nota_ de corrección: Anteriormente, habilitar la JavaScript minificación en el modo de producción dentro del panel de administración causaba que JavaScript aparecieran errores relacionados con TinyMCE 6 en la consola explorador, lo que afectaba al funcionalidad y al experiencia del usuario. Ahora, este problema se ha resuelto, asegurando que TinyMCE 6 funcione sin problemas sin generar ningún error, igualado cuando la minimización de JS está habilitada.
...
### Administración IU, Catálogo

* _ACP2E-2708_: No se pueden cambiar las posiciones de los productos categoría en el sitio web permitido como usuario de administración restringido
   * _Nota_ de corrección: Permitir que un usuario de administrador restringido agregue y ordene productos en un categoría contenido en el categoría raíz asignado en el sitio web restringido.
...
* _ACP2E-3146_: GTM falta el evento addToCart en dataLayer para un producto configurable con opción personalizada
...
* _AC-13501_: El paquete 2.4.8-beta102 Enterprise Edition está fallando con aplicación excepciones
* _ACP2E-2139_: Los productos asignados al catálogo compartido no se reflejan en el front-end cuando se ejecuta el índice parcial
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/7377de59>
* _ACP2E-2873_: [Cloud] Price display en móvil y versión escritorio no es lo mismo en &quot;Mis cotizaciones&quot;
   * _Nota_ de corrección: La línea de impuestos de inclusión innecesaria ya no se muestra en la cotización negociable cuando se gasta la sección de precio total del catálogo.
...
* _ACP2E-3247_: sales_clean_quotes cron elimina las cotizaciones de las órdenes de compra aún aprobadas
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/581b7ef1>
* _ACP2E-3465_: Realizar pedido botón desaparece en los detalles de la orden de compra
   * _Nota_ de corrección: Se ha corregido un problema por el cual el botón Realizar pedido se oculta para los pedidos de compra aprobados cuando una variación de producto tenía un número mínimo en tarjeta especificado
* _ACP2E-3474_: [NUBE] No existe tal entidad con id = 0 con módulo b2b
   * _Nota de la_ corrección: el usuario conectado puede agregar productos a carro de compras cuando las funciones de catálogo compartido están habilitadas.
Anteriormente, al agregar el producto al carro de compras resultaba en Error &#39;no existe tal entidad con id = 0&#39;
* _ACP2E-3562_: No se muestra ningún mensaje de error para nuestros productos en stock al agregar a granel desde el lista de solicitud
   * _Nota_ de corrección: Antes de la corrección, se mostraba un mensaje de éxito independientemente de la cantidad de productos que no se podían agregar al carro de compras. Ahora, se muestran mensajes separados para los productos que se agregaron correctamente al carro de compras y para los productos que fallaron.
* _ACP2E-3628_: Problema con actualizaciones de unidad de almacén después de actualizaciones programadas que causan permisos de producto incorrectos (denegación de -2)
   * _Nota_ de corrección: Modificar el unidad de almacén de un producto con actualizaciones programadas anteriores ya no hace que el producto sea inaccesible para los clientes del catálogo compartido con derecho a ver el producto.

### B2B, Catálogo

* _ACP2E-2860_: Productos/Categorías visibles durante la reindexación cuando se utilizan permisos NoDDL y Categoría
   * _Nota_ de corrección: Evite mostrar en el escaparate categorías restringidas y sus contenido mientras se realiza la indexación de permisos de catálogo.
...
* _ACP2E-3391_: [Nube] No se puede establecer la custom_attributes durante la creación compañía a través de la llamada graphql
   * _Nota_ de corrección: Después de la corrección, es posible establecer el atributo &quot;custom_attributes&quot; para el administrador de compañía durante compañía creación usando graphql solicitud.

### Braintree

* _AC-14293_: la botón de cierre de compra rápido de administración está deshabilitada.
* _BUNDLE-3367_: Pago a través de LPM
   * _Nota_ de corrección: El sistema ahora procesa correctamente los métodos de pago locales (LPM) en la carga inicial, igualado cuando las direcciones de envío y facturación de un cliente conectado no coinciden, lo que garantiza una proceso de cierre de compra sin problemas. Anteriormente, una falta de coincidencia entre las direcciones de envío y facturación de un cliente impedía que LPM se renderizara, causando posibles interrupciones durante el proceso de pago.
* _BUNDLE-3368_: configurable con Virtual como producto secundario
   * _Nota_ de corrección: El sistema ahora permite métodos de pago expresos para productos configurables que tienen un producto hijo virtual, lo que garantiza una proceso de cierre de compra sin problemas. Anteriormente, los métodos de pago expreso no estaban disponibles cuando se agregaba al carro de compras un producto configurable con un producto secundario virtual.
* _BUNDLE-3369_: Error de verificación CVV fallido
* _BUNDLE-3370_: Bóveda a través del área cuenta Problemas 247
   * _Nota_ de corrección: El sistema ahora permite a los clientes guardar nuevos tarjeta o PayPal cuenta información en múltiples sitios web sin encontrar errores de autorización. Anteriormente, los clientes no podían guardar nuevos métodos de pago en diferentes sitios web y se les presentaba un mensaje de error de autorización.
* _BUNDLE-3371_: Envío a una dirección de un país diferente
   * _Nota_ de corrección: El sistema ahora permite que las transacciones se procesen sin errores al enviarse a una dirección de un país diferente, lo que garantiza una proceso de cierre de compra sin problemas. Anteriormente, intentar realizar un envío a una dirección desde otro país provocaba errores de consola, a pesar de que no había errores visibles en la interfaz.
* _BUNDLE-3372_: Tarjeta de crédito - Función de desmontaje
   * _Nota_ de corrección: El sistema ahora maneja correctamente el desmontaje de Braintree componentes de PayPal cuando un cliente navega desde el Página de pago hasta el Página de envío, evitando cualquier error y asegurando que los botones PayPal Express se representen correctamente. Anteriormente, volver al Página de envío desde la Página de pago a veces daba lugar a un error al intentar desmontar el Braintree PayPal componentes.
* _BUNDLE-3373_: devolución de llamada de envío para PayPal Express
...
* _AC-13321_: Magento Excepción lanzada en algún orden relacionado prueba casos
   * _Nota_ de corrección: El sistema ahora maneja correctamente el paso &#39;sendGuestPaymentInformation&#39; en varios casos prueba, evitando que se generen Magento excepciones. Anteriormente, estas excepciones se producían debido a un método de pago nulo, causando fallos en varios casos prueba.
...
* _AC-13797_: Registro de regalos El producto no se muestra correctamente
* _AC-13841_: Registro de regalos El producto no se muestra correctamente
...
* _ACP2E-2664_: [Producción en la nube] Las solicitudes paralelas para añadir mismo producto al carro de compras dan como resultado dos elementos separados en la API de descanso del carro
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/f89a447e>
* _ACP2E-2676_: Problema con el pedido desde el Registro de regalos Magento 2.4.4 Empresa/Comercio
   * _Nota_ de corrección: Se ha resuelto el problema que impedía la compra correcta de un producto de un registro de regalos, lo que permite realizar pedidos y actualizar adecuadamente el registro de regalos. Anteriormente, se producía un error al intentar realizar un pedido desde un registro de regalos, lo que impedía la finalización de la compra.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/35432>
...
* _ACP2E-2917_: [Productos relacionados con la nube] Las reglas no funcionan al cambiar el vista tienda
   * _Nota_ de corrección: El problema se ha solucionado confirmando que el valor de Propiedad personalizado se ha recibido correctamente en el Página carro de compras. Anteriormente, no se buscaba correctamente al cambiar entre tiendas en el escaparate carro de compras Página.
...
* _ACP2E-3402_: El formulario de dirección de envío de pago recibe actualización con la dirección incorrecta
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/078c387e>
* _ACP2E-3405_: [CLOUD] Checkout no conserva la dirección de facturación seleccionada cuando la dirección Search está habilitada
   * _Nota_ de corrección: Los Página de pago de compra ahora conservarán la dirección facturación seleccionada cuando la dirección búsqueda esté habilitada. Anteriormente, si &quot;Límite de direcciones de clientes&quot; estaba configurado en 1 y el cliente tenía más de una dirección, la dirección de facturación seleccionada desaparecía después de volver a cargar el Página.
...
### Carro y cierre de compra, Envío

* _AC-12119_: Compra instantánea &quot;envío más barato&quot; roto para productos configurables
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38819>, <https://github.com/magento/magento2/commit/29fe9097>

### Catálogo

* _AC-10910_: La limpieza de cron_schedule tabla de base de datos no limpia los trabajos inexistentes
   * _Nota de corrección_: El sistema ahora limpia automáticamente la tabla de la base de datos cron_schedule, eliminando las entradas para los trabajos que ya no existen en el sistema. Esto garantiza un rendimiento óptimo manteniendo un número mínimo de filas en la tabla. Anteriormente, las entradas de los trabajos de los módulos inactivos o eliminados no se limpiaban, lo que provocaba una acumulación de datos innecesaria en la tabla cron_schedule.
...
* _AC-10953_: El precio de nivel no se elimina del producto configurable
   ...
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38390>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38427>
* _AC-11804_: descripción Categoría WYSIWYG está vacío en storeview no predeterminado
   * _Nota_ de corrección: El sistema ahora guarda y muestra correctamente la descripción categoría en el editor WYSIWYG al editar un categoría en el nivel vista tienda. Anteriormente, la editor WYSIWYG aparecía vacía después de guardar una descripción categoría en el nivel vista tienda.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38622>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38623>
* _AC-11970_: Imposible reordenar productos configurables con una casilla de verificación seleccionada opción personalizada
...
* _ACP2E-2888_: [ACLARACIÓN] Paquete de problemas de tabla de Secuencia de productos
   * _Nota_ de corrección: Los registros de las tablas de Secuencia del producto del paquete (Secuencia_product_paquete_option, Secuencia_product_paquete_selection) se eliminan cuando se elimina el producto del paquete o se eliminan las opciones del producto del paquete.
Anteriormente, los registros de las tablas de Secuencia del producto Bundle no se eliminaban.
...
* _ACP2E-3076_: Error al eliminar opciones de paquete
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/6a185204>
* _ACP2E-3094_: Categoría problema de permisos de memoria insuficiente explorador
   * _Nota_ de corrección: Categoría IU de permisos se ha rediseñado para permitir el procesamiento de una gran cantidad de permisos mediante componentes y paginación IU listos para usar. Anteriormente, categoría permisos provocaban que explorador bloquearan con una gran cantidad de permisos asignados al categoría.
...
* _ACP2E-3136_: [Los elementos de Categoría Cloud] Sub no se muestran en la edición de widgets en el backend de administración
   * _Nota_ de corrección: El árbol categoría del nuevo widget Página ya no debería tener problemas para cargar categorías de nivel 5+. Anteriormente, faltaban algunas categorías al cargar el árbol más allá de las categorías de Nivel 5.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/148c3ead>
* _ACP2E-3198_: [nube] Problema de zoom y movimiento con dos dedos en el dispositivo móvil real
   * _Nota_ de corrección: El sistema ahora garantiza un funcionalidad de zoom de imagen consistente en dispositivos móviles, proporcionando una experiencia del usuario fluida y predecible. Anteriormente, la función de zoom de imagen era inconsistente y se alejaba repentinamente después de cierto punto cuando se veía en un dispositivo móvil.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/1366ae5e>
* _ACP2E-3282_: Cuando anulamos la asignación de productos del catálogo compartido, los productos de la lista de deseos no se borran
   * _Nota_ de la corrección: Ahora, no hay elementos visibles en la lista de deseos si un producto no está disponible en el catálogo compartido. Anteriormente, la lista de deseos Página mostraba incorrectamente un recuento de &quot;1 artículo&quot; igualado cuando no había artículos realmente disponibles en la lista de deseos.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/5184c067>
* _ACP2E-3286_: Productos relacionados Seleccionar todo/Anular selección de todo Problema
...
* _ACP2E-3469_: Error de validación activada para todos los productos del grupo cuando se tiene una cantidad no válida
   * _Nota_ de corrección: Ahora, el error de validación se activa correctamente para todos los productos del grupo cuando un producto tiene un cantidad de no válido, lo que no sucedía anteriormente.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/56463d5e>
...
* _ACP2E-3533_: Problema de rendimiento en Stock recuperación de cantidad para productos agrupados con múltiples fuentes
   * _Nota de corrección_: Los Página de edición de productos y paquete productos agrupados ahora se optimizan cuando los productos asignados tienen un gran número de fuentes de inventario.
   * _Contribución_ de código de GitHub: <https://github.com/magento/inventory/commit/0208e433>
* _ACP2E-3641_: Reparar https://jira.corp.adobe.com/browse/ACP2E-3389
...
* _ACP2E-3312_: Los precios de nivel devuelven un valor incorrecto en los productos GraphQL (en comparación con Storefront)
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/1366ae5e>
* _ACP2E-3385_: [CLOUD] B2B: problema categoría a través de GraphQL
   * _Nota_ de corrección: Después de la corrección, el consulta de categorías graphql devuelve categorías con permitir permiso igualado si el categoría raíz no tiene permitir permiso.
...
* _ACP2E-3195_: Dirección de envío vacía al realizar un pedido de un artículo de registro de regalos
...
* _ACP2E-2842_: Cambio al modo de tienda único: contenido global ya no aparece
   * _Nota_ de corrección: El sistema ahora sincroniza tienda configuraciones de diseño vista con las configuraciones de diseño del sitio web al habilitar el modo de tienda único, asegurando que las actualizaciones de contenido sean visibles en el frontend. Anteriormente, cambiar al modo de tienda único evitaba que las actualizaciones de contenido se reflejaran en el escaparate.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/7e0e5582>
...
* _ACP2E-3113_: El previsualización de ensayo de contenido en páginas categoría no muestra widgets de producto
   * _Nota_ de corrección: El problema se ha solucionado asegurándose de que las entradas de productos para el categoría adicional vinculado al bloque CMS se hayan registrado con precisión en la base de datos. Anteriormente, devolvía un conjunto de resultados vacío cuando se solicitaba el Página previsualización categoría.
...
* _ACP2E-3326_: Se produce un error en la Página CSM cuando los bloques CMS se insertan utilizando Página generador en cierto orden
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2-page-builder/commit/ae2cdeb0>
* _ACP2E-3388_: [Los bloques dinámicos de nube] no funcionan correctamente
   * _Nota de la_ corrección: Los segmentos de clientes conectados ahora se borran después de cerrar la sesión, lo que impide que la sesión de invitado herede segmentos conectados anteriormente
...
### Contenido, Optimización de los motores de búsqueda

* _ACP2E-2870_: CMS Página jerarquía puede causar problemas de reescritura URL
   * _Nota_ de corrección: Anteriormente, para la reescritura de URL permanente personalizada para páginas raíz que no son sitios web, redirección infinitamente y el Página nunca se cargó. Después de aplicar esta corrección, la reescritura de URL personalizada para el Página raíz que no es un sitio web funciona como se esperaba y no se produce ningún bucle de redirección.

### Contenido, puesta en escena y Vista previa

* _ACP2E-2979_: regla de precio de catálogo no se muestra cuando se configura para programar con bloques dinámicos
   * _Nota de corrección_: El sistema ahora muestra correctamente contenido dinámico asociados con las reglas de precios de catálogo programadas en el Página de detalles del producto. Anteriormente, no se cargaba contenido dinámico cuando se programaban las reglas de precios del catálogo.
...
* _AC-13060_: Segmento de clientes > condición > historial del producto* > &quot;Se vio el producto&quot; no funciona
   * _Nota_ de corrección: El sistema ahora muestra correctamente los clientes registrados coincidentes en la condición &quot;Se vio el producto&quot; en Segmentos de cliente, cuando se cumple la condición. Anteriormente, igualado cuando se cumplía la condición, el recuento de clientes registrados emparejados permanecía en cero.
...
* _AC-10654_: Pregunta/problema de punto final V1/clientes/contraseña
   ...
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38238>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/0c53bbf7>
* _AC-10738_: la configuración de barniz no excluye todos los parámetros marketing
   * _Nota_ de corrección: El sistema ahora excluye correctamente todos los parámetros de marketing comunes en la configuración de barniz, lo que garantiza una seguimiento y análisis precisos. Anteriormente, no se excluían determinados parámetros marketing como gad_source, srsltid y msclkid, lo que provocaba posibles imprecisiones en recopilación de datos.
...
* _AC-10838_: Proceso de indexación de errores del proceso de índice búsqueda catálogo
   ...
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38254>
...
* _AC-10991_: Ser inundado aleatoriamente con consultas de bloques relacionados / impulsar ventas / venta cruzada e indexación de precios
   * _Nota_ de corrección: El sistema ahora optimiza las consultas de bloques relacionados, impulsar ventas y realizar venta cruzada, mejorando el rendimiento y evitando que el sitio se caiga debido a consultas excesivas. Anteriormente, el sistema podía sobrecargarse con consultas de estos bloques, causando ralentizaciones significativas y potencialmente derribando el sitio.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/36667>
...
* _AC-11423_: Excepción: Advertencia: Intentando acceder al desplazamiento de matriz en... -> Calendar.php desde la actualización a ICU 74.1 (PHP Intl)
   * _Nota_ de corrección: Comercio ya no registra la siguiente excepción en el exception.log cada vez que un comprador o comerciante visita el escaparate o el administrador: `main.CRITICAL: Exception: Warning: Trying to access array offset on value of type null in /vendor/magento/framework/View/Element/Html/Calendar.php on line 114 in /vendor/magento/framework/App/ErrorHandler.php:62`. [GitHub-38214](https://github.com/magento/magento2/issues/38214)
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38214>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38364>
* _AC-11476_: [Problema] Solucionar problemas con Datos del cliente cuando el formulario contiene un elemento con el nombre `method`
...
* _AC-11489_: [Corrección de problemas] de PHPDocs para \Magento\Framework\Data\Collection::getItemById
   * _Nota_ de corrección: Los PHPDocs para el método \Magento\Framework\Data\Collection::getItemById se han actualizado para incluir null como posible tipo de retorno, solucionando problemas con herramientas de análisis estáticas. Anteriormente, los PHPDocs del método no especificaban null como un posible tipo de retorno, lo que provocaba advertencias o errores en el análisis estático cuando el método se usaba en declaraciones condicionales.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38485>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38439>
* _AC-11592_: [Problema] Permitir solo preferencias válidas durante la compilación de instalación:di:
   * _Nota_ de corrección: el sistema ahora genera un error durante el comando de compilación de instalación:di:si se crea una preferencia para una clase que no existe o está específicamente excluida, lo que garantiza que solo se permiten preferencias válidas. Anteriormente, estos escenarios fallaban silenciosamente, lo que podía inutilizar cualquier complemento asociado con las clases originales.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38517>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/33161>
* _AC-11651_: Magento intentando modificar Propiedad de solo lectura en __wakeup método de LoggerProxy
   * _Nota_ de corrección: El sistema ahora permite la modificación de propiedades previamente de solo lectura en el método de __wakeup de LoggerProxy, asegurando un funcionamiento sin problemas sin obligar a los usuarios a emplear una solución alternativa. Anteriormente, un intento de reasignar el valor de un Propiedad de solo lectura en el método __wakeup de LoggerProxy causaba problemas.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38526>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/c8f87c25>
...
* _AC-11696_: ChangelogBatchWalker no funciona en varios subprocesos
   * _Nota_ de corrección: El sistema ahora admite la bifurcación del proceso para la indexación de MView, evitando errores durante la ejecución del indexador cuando se opera en varios hilos. Anteriormente, la ejecución de ChangelogBatchWalker en varios subprocesos posible cliente a la eliminación de tablas utilizadas por otros subprocesos, causando un error durante la ejecución del indexador.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38246>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38248>
* _AC-11781_: [Edición] Cambiar nombre erróneamente nombrada variable
...
* _AC-11819_: La caché FPC incorporada se rompe en 2.4.7 para algunas configuraciones
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38646>, <https://github.com/magento/magento2/commit/0c53bbf7>
* _AC-11829_: [Corrección de problemas] que gestionan incoherencias en el manejo de excepciones entre los modos de desarrollador y de producción
   * _Nota_ de corrección: El sistema ahora gestiona de forma coherente las excepciones entre los modos de desarrollador y producción, evitando la redirección inesperada al Página de inicio de sesión cuando se genera una excepción. Anteriormente, una incoherencia en el manejo de excepciones podía provocar que un redirección al inicio de sesión Página en modo de producción en lugar de mostrar el mensaje de excepción.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38639>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/37712>
...
* _AC-13247_: configuración: la actualización está fallando con la versión 11.4 de MariaDB debido a cambios en el conjunto de caracteres y la intercalación
...
* _ACP2E-2966_: En el iPad mini el menú y la cabecera se cargan como móviles, en su lugar deberían cargarse como escritorio.
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/35b1b1da>, <https://github.com/magento/magento2-page-builder/commit/4d5db10a>
* _ACP2E-3046_: Error de tabla base o vista no encontrado al ejecutar mview cron mientras se realiza una operación DDL
   * _Nota_ de corrección: El sistema ahora maneja correctamente las operaciones de actualización de la base de datos mientras mview update se ejecuta en segundo plano, evitando la Ocurrencia de errores &#39;Tabla base o vista no encontrada&#39;. Anteriormente, algunas operaciones de actualización de la base de datos podían dar lugar al error &#39;Tabla base o vista no encontrado&#39;, si la actualización mview se estaba ejecutando al mismo tiempo.
...
* _ACP2E-3375_: [NUBE] En admin hay muchos errores de javascript en la consola
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/d75cff27>
* _ACP2E-3387_: [Cloud] Magento: se ha eliminado cola mensaje
...
* _AC-7976_: [Se ha introducido la] compatibilidad con tipos escalares personalizados para GraphQL esquema
   * _Nota de_ corrección: El sistema ahora admite tipos escalares personalizados para GraphQL esquema, lo que permite a los desarrolladores definir tipos escalares e implementaciones personalizados. Esta característica puede ser especialmente útil para expresar valores que pueden requerir validación, como HTML, correos electrónicos, direcciones URL, fechas, etc., y para casos más avanzados gustar atributos EAV. Anteriormente, el sistema no admitía el procesamiento de tipos escalares personalizados en GraphQL.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/36877>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/34651>, <https://github.com/magento/magento2/commit/0574ac23>

### Marco, Producto

* _AC-13011_: 2.4.8-beta1 EE Los informes no se están generando debido a la excepción de magento
...
* _ACP2E-2642_: caché de Fastly no borrada para la actualización de ensayo de contenido
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/ba25af8a>
* _ACP2E-2653_: Desactivación de la navegación en capas: no elimina la agregación de Graphql
...
* _ACP2E-2928_: La llamada a los productos GraphQL que contiene el filtro de precios {from:&quot;0&quot;} no devuelve ningún resultado
   * _Nota_ de corrección: Anteriormente, los productos graphql búsqueda con filtro para precios cero no devolvían ningún resultado debido a una excepción arrojada. Ahora el búsqueda devuelve los resultados esperados.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/c971859e>
* _ACP2E-2974_: Traducciones para los atributos de devolución del cliente no reflejadas en la API de GraphQL para StoreView respectivo
   * _Nota_ de corrección: Las traducciones de los atributos de devolución del cliente se reflejan en la API de GraphQL para StoreView correspondiente.
Anteriormente, los atributos de devolución del cliente para StoreView respectivo no se reflejaban en la API de GraphQL.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/ec7e32a9>
...
* _LYNX-711_: Esquema de una tabla cuando se crea marca nueva diferente a cuando se actualiza
   * _Nota_ de corrección: Se ha resuelto un problema por el que la adición de una nueva columna VARCHAR a una tabla existente causaba errores prueba debido a esquema diferencias entre instalaciones nuevas y actualizaciones. El método modifyColumn() ahora controla correctamente las columnas VARCHAR estableciendo el conjunto de caracteres y la intercalación predeterminados.
...
* _AC-13242_: Magento actualización falla en MariaDB 11.4 + 2.4.8-beta1
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/7b336d0a>
* _ACP2E-2102_: No hay VCL de exportación para el barniz 7 botón en el panel de administración
   * _Nota_ de corrección: El botón &quot;Exportar VCL para barniz 7&quot; se ha añadido al panel de administración.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/a4fbf702>

### Inventario / MSI

* _AC-10750_: La actualización de inventario del producto configurable falla cuando la base de datos utiliza prefijos
   * _Nota de corrección_: El sistema ahora actualiza correctamente el inventario de productos configurables cuando la base de datos utiliza prefijos, evitando cualquier mensaje de error y asegurando que se guarda el cantidad correcto. Anteriormente, se producía un error al intentar guardar el cantidad de inventario para productos simples dentro de un producto configurable si la base de datos usaba prefijos.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38045>
* _AC-11593_: La clave API de Google no funciona al añadir un mapa con atributos
...
* _ACP2E-1411_: [Productos de paquete de prueba] con 0 inventario en tienda anverso
   * _Nota_ de corrección: El producto paquete no se muestra en los sitios web adicionales que utilizan stock adicional.
* _ACP2E-2794_: [Problema crítico de la nube] con la lista de productos con espacios vacíos
   * _Nota_ de corrección: El sistema ahora muestra correctamente las listas de productos sin espacios vacíos cuando los productos están configurados en &quot;Fuera de Stock&quot;, lo que garantiza una visualización coherente y precisa de los productos disponibles. Anteriormente, establecer un producto en &quot;Fuera de Stock&quot; resultaba en un espacio vacío que aparecía en la lista de productos, lo que interrumpía el diseño y podía confundir a los clientes.
...
* _ACP2E-3335_: No se puede enviar el pedido cuando el tienda de recogida MSI está habilitado
   * _Nota_ de corrección: Rendimiento inventario mejorado de crear envío en caso de muchas fuentes con recogida en tienda
   * _Contribución_ de código de GitHub: <https://github.com/magento/inventory/commit/9f3e63d1>
* _ACP2E-3355_: El reindex cron no actualiza la disponibilidad del producto en el frontend
...
* _AC-10994_: [Problema] ID de tienda incorrecto utilizado en el procesador de direcciones de pedido
...
* _ACP2E-3425_: el administrador aún puede realizar pedidos igualado sin método de pago
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/d50f6b5d>
* _ACP2E-3518_: Items are duplicated after we create an order from Admin on - Mozilla Firefox explorador
   * _Nota_ de corrección: Los productos añadidos mediante &quot;añadir Products by unidad de almacén&quot; ya no se duplican en Firefox al crear un pedido en la administración.
...
* _PAQUETE-3394_: [Braintree] reembolso en línea almacenar transacción como transactionid-refund
* _PAQUETE-3421_: [pedidos de Braintree] + [CLOUD] Braintree (tarjeta crédito) no se pueden dividir los cargos
* _BUNDLE-3422_: [el certificado SSL de Braintree Braintree] [Cloud]caduca el 30 de junio
...
* _LYNX-366_: Error de servidor en props de correo electrónico en consultas de tarjeta de regalo físico
   * _Nota_ de corrección: Se ha corregido un problema por el que las consultas de sender_correo electrónico y destinatario_correo electrónico en tarjetas regalo físicas provocaban un error de servidor. Estas props ahora se devuelven correctamente para las tarjetas regalo virtuales y el comportamiento consulta es coherente.
...
* _LYNX-395_: only_x_left_in_stock atributo en ProductInterface no es preciso en productos configurables
   * _Nota_ de corrección: Se ha corregido un problema por el que el atributo only_x_left_in_stock de ProductInterface no reflejaba con precisión el stock disponible para las variantes de producto configurables en el carro de compras. Ahora, el valor only_x_left_in_stock corresponde correctamente a los niveles de stock de la variante real, lo que garantiza que se devuelvan datos de inventario precisos en el consulta de Cart GQL.
...
* _LYNX-405_: GraphQL Error: tipo de &#39;archivo&#39; no compatible en opciones personalizables consulta
   * _Nota_ de corrección: Se ha corregido un problema por el que GraphQL devolvía un error para opciones personalizables de tipo &quot;archivo&quot; en carro de compras elementos. La consulta ahora devuelve correctamente los detalles de todos los tipos de opciones personalizables, incluidas las opciones basadas en archivos, sin causar errores.
...
* _LYNX-421_: No se puede agregar cupón a carro de compras para el descuento de solo envío
   * _Nota_ de corrección: Se ha corregido un problema por el que no se podía aplicar un cupón a un carro de compras para descuentos solo de envío. El cupón ahora se aplica correctamente al importe del envío cuando se utiliza un regla de ventas sin condiciones de producto, asegurando que el descuento esperado se aplique al costo de envío.
...
* _LYNX-627_: CustomerOrder.items_eligible_for_return lista debe ser coherente con los elementos del pedido
   * _Nota_ de corrección: Se han resuelto incoherencias con la idoneidad de devolución en pedidos:
1. El lista CustomerOrder.items_eligible_for_return ahora es coherente con los elementos del pedido real.
2. El campo OrderItemInterface.eligible_for_return devuelve correctamente &quot;false&quot; cuando ya se ha devuelto todo el cantidad.
3. CustomerOrder.items_eligible_for_return ahora incluye solo los artículos que aún no están en proceso de devolución.
* _LYNX-628_: añadir campo cantidad_return_requested
   * _Nota_ de revisión: se ha agregado el campo cantidad_return_requested a OrderItemInterface, lo que permite identificar el cantidad de los elementos para los que se ha enviado una devolución. Esto mejora la seguimiento de retorno junto con el campo existente cantidad_returned.
* _LYNX-634_: Las acciones de pedido disponible no deben contener RETURN después de las devoluciones creadas para todos los artículos en su totalidad cantidad
   * _Nota_ de corrección: Se ha corregido un problema por el que el campo de available_actions en el consulta customer.orders de GraphQL incluía incorrectamente RETURN después de que se creara una devolución completa para todos los artículos. La acción RETURN ahora se elimina correctamente una vez que se completa el proceso de devolución.
...
* _ACP2E-3441_: No se pueden descargar archivos desde el recopilador de datos
   * _Nota_ de corrección: Al descargar copia de seguridad ya no se muestra la Página en blanco en lugar de descargar el archivo.
...
* _ACP2E-2673_: Rendimiento de indexación parcial de precio
   * _Nota_ de corrección: Se ha mejorado el rendimiento de indexación parcial de precio optimizando algunas de las consultas de eliminación utilizadas en el proceso de indexación.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/ba25af8a>
* _ACP2E-2850_: El pedido se rechaza en la configuración multi tienda cuando se utiliza el procesamiento de pedidos asíncronos + Términos y condiciones
   * _Nota_ de corrección: Los pedidos realizados desde sitios web no predeterminados con términos y condiciones habilitados ahora se procesan.
Antes de que fueran rechazados automáticamente.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/57a32313>
* _ACP2E-2910_: La llamada a la API de reposo del pedido tarda mucho tiempo en ejecutarse
   * _Nota_ de corrección: El sistema ahora ejecuta la llamada a la API de reposo de pedidos dentro de un periodo de tiempo razonable, mejorando el rendimiento al capturar un gran número de pedidos. Anteriormente, la llamada a la API de descanso del pedido tardaba mucho tiempo en ejecutarse, lo que provocaba retrasos al recuperar un gran número de pedidos.
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/001e5188>

### Rendimiento, promoción

* _ACP2E-2617_: El indexador de regla ventas dejó de funcionar
   * _Nota_ de corrección: El sistema ahora completa correctamente el igualado del indizador de regla ventas con un gran número de grupos de filtros combinados, lo que garantiza que carro de compras condiciones de regla se apliquen al carro de compras como se espera. Anteriormente, el indizador de regla ventas no se completaba cuando había un gran número de grupos de filtros combinados, lo que provocaba un mensaje de error y evitaba el aplicación de carro de compras regla condiciones.

### Precios

* _AC-11810_: Magento2.4.6-p4 API de pedido Falta precio del artículo simple
...
* _AC-13855_: Error de redondeo de centavos en el catálogo regla
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/276e0acd>
...
* _AC-10535_: Los caracteres especiales del nombre de producto asociado configurable se están convirtiendo en entidades HTML.
   * _Nota_ de corrección: El sistema ahora retiene correctamente los caracteres especiales en los nombres de los productos asociados al editar un producto configurable, evitando que se conviertan en entidades HTML. Anteriormente, los caracteres especiales en los nombres de producto asociados se convertían en entidades HTML cuando se editaba el producto configurable.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38146>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38447>
* _AC-10947_: La función GetById de ProductRepository no crea la clave de caché correcta
...
* _AC-11992_: [Problema] [MFTF] añadido AdminClickAddOptionForBundleItemsActionGroup
   * _Nota_ de corrección: El sistema ahora incluye AdminClickAddOptionForBundleItemsActionGroup, mejorando la funcionalidad del panel de administración. Anteriormente, el grupo de esta acción no estaba disponible.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/30857>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/30838>
* _AC-13173_: [Problema] Corrección de errores tipográficos en el bloque PHPDoc
   * _Nota_ de corrección: El sistema ahora elimina correctamente un variable referenciado desconocido en PHPDoc para la declaración de variable de $helper, mejorando la claridad y precisión del código. Anteriormente, esta variable desconocida a la que se hace referencia en PHPDoc estaba causando confusión y posibles imprecisiones en el código.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/38961>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/pull/38940>
* _AC-13423_: [Problema] Se ha corregido el diseño roto de páginas de productos descargables y de paquetes en Magento >= 2.4.7
   * _Nota_ de corrección: Se ha corregido el diseño de las páginas de productos paquete y descargables, lo que garantiza una visualización coherente y correcta en todos los dispositivos. Anteriormente, estas páginas experimentaban problemas de diseño debido a una reorganización del bloque de medios de información del producto.
   * _Problema_ de GitHub: <https://github.com/magento/magento2/issues/39403>
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/6cfb9b6b>
...
* _ACP2E-2944_: [La función Cloud] addFilterToMap no funciona para ciertas columnas
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/3a7c4d17>
* _ACP2E-3471_: [Productos de nube] en Categoría - Productos añadir - Asignar - Seleccionar todo
   * _Nota_ de la corrección: Los usuarios ahora pueden seleccionar o anular la selección de productos mediante el botón de alternancia.
...
* _ACP2E-2811_: [Regla de catálogo de reindexación en la nube] El indexador de productos lanza SQLSTATE[HY000]: Error general: 2006 El servidor MySQL ha desaparecido.
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/b2286ecf>
* _ACP2E-2926_: [Regla de precio del carro de compras de CLOUD]para visitantes Segmento de clientes que no aplica descuento en carro de compras
   * _Nota de corrección_: El sistema ahora aplica correctamente las reglas de precios del carro de compras para los segmentos de clientes visitantes, igualado si el regla no utiliza un cupón, asegurándose de que se apliquen los descuentos adecuados al carro de compras. Anteriormente, los descuentos no se aplicaban a la carro de compras para los segmentos de clientes visitantes, a menos que la regla de precio del carro usara un cupón.
* _ACP2E-3024_: falta el atributo &quot;type&quot; en la pestaña &quot;Products to Match&quot; de las reglas de productos relacionadas
   * _Nota_ de corrección: El atributo &quot;Tipo&quot; ahora está disponible como una opción de filtro en la pestaña &quot;Productos para coincidir&quot; del módulo &quot;Reglas de productos relacionados&quot;, lo que permite una definición de regla más precisa. Anteriormente, este atributo faltaba en el pestaña &quot;Productos para coincidir&quot;, lo que limitaba la capacidad de crear criterios de coincidencia precisos.
* _ACP2E-3139_: La regla de ventas con el atributo Discount Qty Step (Buy X) hace que no se apliquen otras reglas
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/d01ee51e>
* _ACP2E-3331_: Problema de rendimiento en el precio del carro de regla - Regla de ventas anticipadas módulo
   * _Nota_ de corrección: Se han añadido los índices de base de datos que faltaban para AdvancedSalesRule filtros
* _ACP2E-3332_: Emitir reglas de venta con descuento de cantidad fija y &quot;Se aplica el descuento por cantidad máxima&quot;
   ...
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/581b7ef1>
* _ACP2E-3342_: [La actualización del Magento CLOUD] hacía que los cupones hicieran distinción entre mayúsculas y minúsculas
   * _Nota_ de corrección: Antes de la corrección, se le pedía que escribiera el código de cupón exactamente como estaba configurado, tomando consideración mayúsculas o minúsculas. Ahora, la cupón se validará en el backend independientemente de la configuración del código en mayúsculas o minúsculas.
...
* _ACP2E-3330_: [NUBE] Los usuarios administradores restringidos pueden ver el menú de retorno y los botones
   * _Nota_ de corrección: Los usuarios administradores restringidos ahora no tienen acceso a los controles relacionados con RMA (menú y botones).
Los usuarios administradores anteriormente restringidos podían ver el menú de retorno y los botones.
* _ACP2E-3443_: La pantalla de retorno está desordenada al actualizar la pantalla
   * _Nota_ de corrección: El usuario puede actualizar el Página sin experimentar distorsión de pantalla.
...
* _AC-13751_: No se aplica la regla del precio del segundo carro de compras si ya se ha aplicado el primer regla del carro
...
* _AC-12938_: Actualizaciones de instrucciones de configuración &quot;sandbox&quot; y &quot;prod&quot; de UPS REST en devdoc
...
* _ACP2E-2901_: La configuración de actualización programada no se guarda si se agregó originalmente al ejecutar la actualización
   * _Nota_ de corrección: El sistema ahora borra correctamente los valores de atributos del producto en las actualizaciones programadas posteriores cuando dichos atributos se modifican en la actualización que se está ejecutando. Anteriormente, cuando una actualización programada en ejecución modificaba un atributo de producto, era imposible borrar dichos valores de atributos al crear una nueva actualización programada, lo que requería que el usuario los volviera a editar después de la creación.
* _ACP2E-2999_: Regla de precio del carro de compras desde la fecha y hasta el problema no sincronizado con la actualización de ensayo
   * _Nota_ de corrección: Las fechas se guardan según las actualizaciones del ensayo de la regla de precios del carro de compras.
* _ACP2E-3104_: Error JS en el Vista previa de ensayo
   * _Nota_ de corrección: Ahora el archivo form-mini-stub.js se carga correctamente sin ningún error de sintaxis Js en las herramientas de desarrollo.
* _ACP2E-3162_: Precio especial del producto No se puede actualizar el contenido escalonado
   * _Nota_ de corrección: El sistema ahora permite la edición de la fecha de finalización de una actualización de precios campaña después de que haya comenzado, asegurando que los usuarios puedan realizar los ajustes necesarios en sus campañas. Anteriormente, se producía un error al intentar actualizar la fecha de finalización de un campaña activo, lo que impedía a los usuarios realizar cambios.
...
* _ACP2E-3458_: [MFTF] StorefrontCheckoutProcessForQuoteWithoutNegotiatedPricesTest
   * _Nota_ de corrección: mftfs fijos
   * _Contribución_ de código de GitHub: <https://github.com/magento/magento2/commit/078c387e>
