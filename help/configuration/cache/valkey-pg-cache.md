...
- **Caché Zend (2.4.8 y anteriores)** — Servidor Valkey para Commerce 2.4.8 y anteriores:
  - **Servidor Valkey basado en Zend** — Utiliza la ruta de clase completa (`Magento\Framework\Cache\Backend\Valkey`)
...
- **Caché Symfony (2.4.9+)**: a partir de Commerce 2.4.9, la caché Symfony proporciona una implementación de caché compatible con PSR-6 para Valkey con mejoras de rendimiento significativas:
...
Adobe Commerce proporciona la opción `allow_parallel_generation` a los usuarios que desean eliminar la espera de bloqueos. Está desactivada de forma predeterminada y Adobe recomienda desactivarla hasta que tenga configuraciones o bloques excesivos.
