...
Todas las operaciones asincrónicas de Commerce se realizan con el comando Linux `cron`. Consulte [Configurar y ejecutar cron](../configuration/cli/configure-cron-jobs.md) para configurarlo correctamente.
...
* Le recomendamos que utilice herramientas de terceros para la minificación y el agrupamiento (como [r.js](https://requirejs.org/)). Los mecanismos integrados de Commerce no son óptimos y se envían como alternativas de reserva.
...
Las acciones del cliente configuran un proceso de validación para los segmentos del cliente, que es lo que puede causar una degradación del rendimiento. De forma predeterminada, Adobe Commerce valida cada segmento en tiempo real para definir qué segmentos del cliente coinciden y cuáles no.
