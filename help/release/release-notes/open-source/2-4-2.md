...
description: Obtenga información sobre las nuevas funciones, mejoras, correcciones de errores y problemas conocidos en la versión del Magento Open Source 2.4.2.
...
Magento Open Source 2.4.2 presenta mejoras en el rendimiento y la seguridad, además de mejoras significativas en la plataforma. Las mejoras de seguridad incluyen la ampliación de la compatibilidad con el `SameSite` para todas las cookies. Ahora se admiten Elasticsearch 7.9.x y Redis 6.x.
...
* Las operaciones del sistema de archivos en los componentes se han estandarizado y endurecido para evitar cargas maliciosas.
...
Esta versión contiene mejoras en la calidad del núcleo, que mejoran la calidad del marco y de estas áreas funcionales: Cuenta del cliente, Catálogo, CMS, OMS, Importar/Exportar, Promociones y Segmentación, y Carro y cierre de compra.
...
* La capacidad de configurar una instalación para utilizar una base de datos dividida ha quedado obsoleta en esta versión. Consulte la [Obsolescencia de la funcionalidad dividida de la base de datos en el Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog.
...
* La aplicación ya no genera un error al intentar cambiar `backend-frontname` usando la variable `ssh` después de instalar el Magento Open Source. [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* La aplicación de una regla de precio con un cupón mediante GraphQL ahora funciona según lo esperado cuando el **Descuento de cantidad fija para todo el carro** se utiliza.
...
* La aplicación ya no enumera un producto descargable en la pestaña Mis productos de descarga después de que el pedido al que pertenece se haya reembolsado parcialmente. [GitHub-28388](https://github.com/magento/magento2/issues/28388)
...
* La resolución de URL de GraphQL ahora gestiona correctamente los parámetros de consulta. Anteriormente, devolvía NULL al gestionar direcciones URL compatibles con SEO con parámetros.

<!-- ENGCOM-8422 -->

* La resolución de URL de GraphQL ahora gestiona correctamente la jerarquía de páginas de CMS. Anteriormente, devolvía NULL. [GitHub-30474](https://github.com/magento/magento2/issues/30474)

<!-- MC-38209-->

* Consultas de un producto `categories.breadcrumbs` los datos ya no devuelven datos de ruta de exploración para categorías cuya categoría principal está deshabilitada. [GitHub-30468](https://github.com/magento/magento2/issues/30468)

<!-- MC-38407-->

* La ruta a una imagen de categoría ahora se devuelve correctamente cuando se desactivan las reescrituras de URL.
...
* Las posiciones de los productos ahora se actualizan según lo esperado en los resultados de la consulta de GraphQL cuando se almacenan en caché los resultados de la consulta.
...
* La variable `FlushCacheByTags` se ha actualizado para que utilice complementos posteriores en lugar de complementos. [GitHub-29558](https://github.com/magento/magento2/issues/29558)
...
* Se ha mejorado el tiempo de respuesta de GraphQL para las operaciones de adición al carro de compras.
...
* Magento Open Source ha mejorado el rendimiento de la caché al eliminar el vaciado de etiquetas varias veces por cada instancia de indexador. [GitHub-29890](https://github.com/magento/magento2/issues/29890)
...
* El Elasticsearch ya no genera un error de modificador desconocido cuando un comprador busca en un sinónimo de búsqueda. Anteriormente, el escape carecía de un símbolo de barra oblicua en las expresiones regulares, lo que daba como resultado un error fatal.
...
**Problema**: La variable `[magento_root]/index.php` se ha eliminado y la aplicación ahora se ejecuta desde `/pub` de forma predeterminada para las configuraciones de Apache. Las tiendas que se proporcionan desde subcarpetas no funcionarán del modo esperado y podrían mostrar errores 404. **Solución alternativa**: Utilice enlaces simbólicos para emular la instalación en una subcarpeta. El siguiente ejemplo utiliza `https://shop01.com/shop/` para ilustrar cómo utilizar un enlace simbólico para emular una instalación en una subcarpeta.
...
Puede instalar el Magento Open Source 2.4.2 mediante [Compositor](../../../installation/composer.md).
