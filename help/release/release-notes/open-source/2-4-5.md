...
## Magento Open Source 2.4.5 resaltado

Busque los siguientes aspectos destacados en esta versión.

### Mejoras de seguridad

Esta versión incluye 20 correcciones de seguridad y mejoras de seguridad de la plataforma. Esta corrección de seguridad se ha transferido al Magento Open Source 2.4.4-p1 y al Magento Open Source 2.3.7-p4.
...
Page Builder v.1.7.2 es compatible con el Magento Open Source 2.4.5.
...
* Las vistas previas de los botones PayPal, Crédito y Pagar más tarde ya están disponibles en Administración para las páginas de producto, minicarro, carrito y cierre de compra. Las vistas previas revelan el aspecto que tendrán estos botones cuando estén activados y procesados en la tienda.

#### Braintree

* El Braintree ha interrumpido la integración de protección contra fraudes de KOUNT. Se ha eliminado de la base de código del Magento Open Source.
...
Para obtener información sobre las mejoras y correcciones de errores, consulte [Versiones de PWA Studio](https://github.com/magento/pwa-studio/releases). Consulte [Compatibilidad de versiones](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) para obtener una lista de las versiones de PWA Studio y sus versiones principales de Magento Open Source compatibles.

## Problemas solucionados

Se han corregido cientos de problemas en el código principal de Magento Open Source 2.4.5.

### Instalación, actualización, implementación

<!-- ACP2E-49-->

* Ahora puede cambiar el nombre de un parche de datos y agregar el antiguo nombre de la clase como un alias en el `patch_list` tabla de base de datos. {{ site.data.var.Ce }} ahora comprueba si ya existían alias de parches de datos en la base de datos antes de aplicar el parche. Anteriormente, applicationOpen Source generaba un error con estas condiciones.

<!-- AC-3036-->

* El Magento Open Source ya no genera una excepción cuando intenta cambiar la dirección URL de administración a una dirección URL personalizada desde el administrador. Anteriormente, después de cambiar la dirección URL de administración, no se podía iniciar sesión. [GitHub-35416](https://github.com/magento/magento2/issues/35416)
...
* El Magento Open Source ahora calcula correctamente el total del carro de compras de un producto de paquete cuando se aplica la regla de subselección de producto.

### Caché

<!-- ACP2E-684-->

* La caché de página completa ya no se muestra como deshabilitada en el Administrador cuando se vacía la caché del Magento Open Source y `use_stale_cache` está activada.

<!-- AC-2410-->

* Los nuevos marcadores de implementación de Reic ahora funcionan según lo esperado cuando se descarga la caché. [GitHub-32649](https://github.com/magento/magento2/issues/32649)
...
* El Magento Open Source ahora proporciona mensajes de error de validación cuando intenta añadir una clave de URL de producto con un guión final. También hay disponible información informativa sobre herramientas.

<!-- ACP2E-267-->

* El nuevo `ConfigurableWishlistItem.configured_variant` el campo ha reemplazado al `ConfigurableWishlistItem.child_sku` campo . Este último campo activaba un error interno cuando una lista de deseos del cliente contenía un producto configurable no configurado.
...
* Cuando la variable `Synchronize widget products with backend storage` está activada, la aplicación Open Source agrega datos de vista de producto recientemente a la `catalog_product_frontend_action` tabla de base de datos. Incluye el ID de cliente o visitante al añadir registros. La variable `recently_viewed_product` ahora está vacía si el ID de cliente y el ID de visitante son nulos. Como resultado, cuando la variable `customer/section/load` Se envía la solicitud de Ajax, la aplicación de código abierto puede filtrar correctamente los productos visualizados recientemente en función del ID de cliente o visitante. Anteriormente, la respuesta incluía todos los datos disponibles en la variable `catalog_product_frontend_action` base de datos porque no se ha comprobado si hay un ID de visitante o cliente vacío.
...
* Las listas de productos ahora se representan correctamente en Admin. Anteriormente, la lista de productos no se representaba y el Magento Open Source mostraba este error:  `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`. [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* Ahora puede cambiar el límite de productos por página que se muestra dentro de una categoría al **Recordar paginación de categoría**  está activada. Anteriormente, la cookie `form_key` y la interfaz de usuario `form_key` y el Magento Open Source mostraron este error:  `Invalid Form Key. Please refresh the page`.
...
* El Magento Open Source ahora muestra un mensaje de error como se espera cuando intenta crear un atributo desde la página del producto sin completar el campo Administrador . [GitHub-33099](https://github.com/magento/magento2/issues/33099)
...
* El Magento Open Source ahora muestra el precio de producto correcto para un producto configurable con una opción seleccionada después de cambiar su cantidad en la página de detalles del producto. Anteriormente, el precio se restablecía al valor inicial después de cambiar la cantidad.
...
* Los atributos de selección múltiple ahora se guardan correctamente durante la edición del producto. Anteriormente, applicationOpen Source guardaba la opción predeterminada para los atributos no seleccionados, así como los atributos seleccionados al guardar un producto.

<!-- ACP2E-55-->

* El Magento Open Source ahora muestra los atributos configurables como se espera durante la creación de `select` mediante una secuencia de comandos de parche. Anteriormente, los atributos globales elegibles estaban ocultos.

<!-- AC-2521-->

* La variable `addConfigurableProductsToCart` ahora se puede utilizar la mutación para añadir productos configurables con opciones personalizadas. Anteriormente, applicationOpen Source generaba este error: `Magento 2.3.4 graphql Notice: Undefined index: option_value in /var/www/html/mg234/vendor/magento/module-configurable-product-graph-ql/Model/Resolver/ConfigurableCartItemOptions.php on line 62`. [GitHub-28860](https://github.com/magento/magento2/issues/28860)
...
* El Magento Open Source ahora muestra correctamente los atributos del sistema EAV predefinidos según la configuración del sitio web en la tienda. Anteriormente, los atributos de cliente a nivel de sitio web que estaban habilitados para un sitio web y deshabilitados para otro se mostraban como habilitados para ambos.
...
* La variable `updateCartCurrency` ahora establece una cadena en lugar de un objeto dentro del objeto cart. Anteriormente, la aplicaciónOpen Source no cargaba una cotización utilizando `getQuote` porque la variable `updateCartCurrency` define un objeto en lugar de una cadena dentro del objeto cart. [GitHub-34199](https://github.com/magento/magento2/issues/34199)
...
* El Magento Open Source ya no muestra un `preg_replace()` en el Administrador. El tercer argumento (`$subject`) es ahora de tipo `array|string` en lugar de `bool`.
...
* Magento Open Source ya no genera el siguiente error al crear un complemento para cualquier método de clase `vendor/magento/module-backend/Model/Menu.php`: `Error: Call to undefined method ReflectionUnionType::getName()`. [GitHub-35292](https://github.com/magento/magento2/issues/35292)

<!-- AC-1518-->

* Ahora, el Magento Open Source devuelve un error 404 en lugar de un error 500 al navegar a `/checkout/sidebar/updateItemQty/?item_qty=error` en la tienda. Anteriormente, se producía este error:  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`. [GitHub-34380](https://github.com/magento/magento2/issues/34380)

<!-- ACP2E-847-->

* Magento Open Source ya no déclencheur a `trim(): Passing null to parameter #1 ($string) of type string is deprecated` cuando la conexión AMPQ está configurada sin la configuración SSL.
...
* El Magento Open Source ya no genera una excepción al agregar un producto de paquete a través de Page Builder.
...
* La variable `products` La consulta ahora devuelve información del producto que refleja con precisión la configuración &quot;Mostrar productos relacionados&quot;. La variable `related_products`, `upsell_products`y `crosssell_products` Los campos de la interfaz de producto de GraphQL ahora se resuelven según la configuración Mostrar productos relacionados, Mostrar productos de ventas superiores y Mostrar productos de venta cruzada, respectivamente.
...
* El Magento Open Source ahora muestra un mensaje de error al establecer un dominio de cookie no válido (**Tienda**  > **Configuraciones**  >  **Web**  >  **Dominio de la cookie de configuración de cookie predeterminada**). Anteriormente, el sitio web se bloqueaba. [GitHub-35048](https://github.com/magento/magento2/issues/35048)

<!-- AC-2765-->

* El Magento Open Source ya no genera un error cuando un administrador con funciones incluidas en un único sitio web agrega producto a Elementos de contenido mediante PageBuilder. Anteriormente, applicationOpen Source generaba un error SQL.
...
* El Magento Open Source ya no genera un error al activar el  **Consulte aquí para vincular una fuente RSS a su lista de deseos** casilla de verificación antes de hacer clic en **Compartir lista de deseos**. [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* La variable `categories` query ya no genera una excepción al obtener una lista de categorías una de las cuales contiene una imagen que no se puede encontrar en el sistema de archivos. Anteriormente, applicationOpen Source arrojaba esta excepción: `Category image not found`. [GitHub-34266](https://github.com/magento/magento2/issues/34266)
...
* Las sesiones de Magento Open Source ya no finalizan después de realizar una solicitud de GraphQL. Anteriormente, la variable `ClearCustomerSessionAfterRequest` plugin ha cerrado la sesión del comprador. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* `setShippingAddressesOnCart` las solicitudes de ahora validan correctamente los ID de región. Anteriormente, el código fuente abierto de la aplicación generaba un error al utilizar el ID de región en lugar del código de región.
...
* `products` consultas ya no devuelve `price_range` para productos configurables afectados por variantes deshabilitadas. [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* El Magento Open Source ahora comprueba la existencia de una vista personalizada antes de filtrar las columnas al exportar informes. Anteriormente, los informes exportados no tenían en cuenta las vistas personalizadas y las columnas exportadas eran incorrectas.

<!-- ACP2E-502-->

* Ahora, el Magento Open Source importa correctamente imágenes con nombres de archivo largos. Anteriormente, applicationOpen Source no importaba la imagen y daba este error: `Imported resource (image) could not be downloaded from external resource due to timeout or access permissions in row(s):`.
...
* Se agregó la validación de los nombres de categoría durante la importación. Anteriormente, la aplicaciónOpen Source no validaba nombres de categoría, lo que provocaba errores cuando los nombres de categoría superaban los 255 caracteres.
...
* Las acciones de cliente, dirección de cliente y pedido ahora se registran correctamente en el informe de acción del administrador. Anteriormente, el código de fuente abierto de la aplicación no registraba acciones si la variable `postDispatch` no se ha especificado el controlador en los ajustes de configuración.
...
* El Magento Open Source ya no cambia las direcciones de correo electrónico personalizadas asignadas a pedidos al cambiar la dirección de correo electrónico principal asignada al cliente en la página de edición de la cuenta de administrador. Anteriormente, cuando editaba la dirección de correo electrónico principal de un cliente, la nueva dirección de correo electrónico se asignaba a cada pedido creado para ese cliente. [GitHub-34397](https://github.com/magento/magento2/issues/34397)

<!-- AC-2664-->

* El Magento Open Source ahora muestra registros del almacén solicitado en la página de cuadrícula de las notas de crédito en implementaciones que ejecutan PHP 7.4. Anteriormente, applicationOpen Source generó el siguiente error después de crear una nota de crédito e intentar verlo: `The store that was requested wasn't found. Verify the store and try again`.

<!-- AC-2442-->

* El Magento Open Source ahora muestra notas de crédito en la página de cuadrícula de notas de crédito para los pedidos creados a partir de vistas de almacén cuyo nombre va precedido de números. Anteriormente, applicationOpen Source mostraba el error: `The store that was requested wasn't found. Verify the store and try again. Exception in /var/www/html/vendor/magento/module-store/Model/StoreRepository.php:75`. [GitHub-35122](https://github.com/magento/magento2/issues/35122)

<!-- ACP2E-31-->

* El Magento Open Source ahora muestra el coste de envío gratuito (0) en los totales de la página de facturas de administración y tienda. Anteriormente, cuando el envío era cero para un pedido, la aplicación Open Source no mostraba el importe de envío en total en el total de envío de la página de factura.
...
* Los administradores ahora pueden realizar un pedido en el administrador mediante el método de pago PayPal PayflowPro. Anteriormente, el applicationOpen Source mostraba este error: `No such entity with cartId = 0`.
...
* Los comerciantes ahora pueden enviar un reembolso parcial por pedidos pagados con Apple Pay a través del Braintree. Este fue un problema conocido en el Magento Open Source 2.4.4.
...
* La variable **Tienda** >  **Configuración** > **Servicios** La página ahora muestra la información de API web de Magento Open Source como se espera cuando el acceso a recursos está establecido en  **Personalizado** en la ficha Recursos de rol . [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* Las consultas de Elasticsearch ahora funcionan según lo esperado cuando `int` está configurado como servidor en el que se pueden realizar búsquedas `type` atributo. Anteriormente, la aplicaciónOpen Source lanzó un `Elasticsearch\Common\Exceptions\BadRequest400Exception` excepción.
...
* El Magento Open Source ahora muestra un recuento preciso de sugerencias de resultados de búsqueda en la tienda en implementaciones en las que Sugerencias de búsqueda y la variable **Mostrar recuento de resultados para cada sugerencia** están activados. Anteriormente, el recuento mostrado al lado de las palabras clave era cero.
...
* Se corrigieron errores de PHP en la variable `catalogsearch/advanced/result` y  `catalogsearch/advanced/index` páginas.  Anteriormente, el Magento Open Source mostraba este error cuando se pasaba una matriz en cualquier cadena de búsqueda avanzada:  `Warning: trim() expects parameter 1 to be string, array given | magento/module-catalog-search`. [GitHub-33586](https://github.com/magento/magento2/issues/33586)

### Envío

<!-- AC-2052-->

* El Magento Open Source ya no genera un error al cargar las tarifas de envío de UPS si no se seleccionan los métodos de envío permitidos. Anteriormente, cuando un comprador introducía una dirección de envío en el flujo de trabajo de cierre de compra bajo estas condiciones, no se mostraban otros métodos de envío y el Magento Open Source mostraba un error en la tienda. [GitHub-34411](https://github.com/magento/magento2/issues/34411)
...
* Ahora, el Magento Open Source actualiza el precio del nivel de exclusión de impuestos para un producto simple en la página del producto, tal como se espera después de que haya cambiado la cantidad del producto simple.
...
* El Magento Open Source ahora muestra un mensaje de error informativo cuando un administrador intenta guardar una dirección con líneas de calle excesivas en Admin **Tienda** > **Atributos**  > **Dirección del cliente**.  El administrador ahora puede eliminar la información de dirección adicional y guardar correctamente la dirección. Anteriormente, la aplicaciónOpen Source comprometía las líneas adicionales, pero no guardaba los datos.
...
* Ahora puede configurar un vídeo de Vimeo para que se ejecute en segundo plano en un `banner` elemento cuando CSP está establecido en `restrict mode`. Anteriormente, applicationOpen Source generaba un error de JavaScript.
...
Puede instalar el Magento Open Source 2.4.5 mediante [Compositor](../../../installation/composer.md).
