...
* Los cambios de estado del pedido ahora se registran según lo esperado en **Sistema** > **Registros de acciones** > **Informe**.

<!-- MC-29637-->

* La aplicación ahora registra las acciones de envío de ventas en el registro de acciones de administración como se espera.

<!-- MC-29892-->

* Las acciones de guardado de la página de CMS ahora se registran en los registros de acciones de administración. Anteriormente, solo se registraban las acciones de vista.

<!-- MC-30618-->

* Las acciones de guardado en las páginas de CMS ahora se registran como se espera en los registros de acciones de administración cuando **Seleccionar todas las acciones que se van a registrar** está habilitado en la ficha Registro de acciones de administración (**Administrador** > **Almacenes** > **Configuración** > **Avanzadas**).

<!-- ENGCOM-7274-->

* Las modificaciones de la página de CMS ahora se registran según lo esperado en el registro de acciones. MFTF ahora cubre esta función.  _Corrección presentada por Lukasz Bajsarowicz en la solicitud de extracción [27597](https://github.com/magento/magento2/pull/27597)_. [GitHub-171](https://github.com/magento/magento2/issues/171)
...
* Adobe Commerce `cron`los nombres de trabajo ahora se incluyen según lo esperado dentro de los trazos de transacciones en NewRelic APM Monitoring - Transactions. Esto incluye `cron` trabajos que se ejecutan desde la línea de comandos, así como trabajos ejecutados por el planificador estándar.  _Corrección presentada por Lukasz Bajsarowicz en la solicitud de extracción [25957](https://github.com/magento/magento2/pull/25957)_. [GitHub-22047](https://github.com/magento/magento2/issues/22047)
...
* Se ha mejorado la lógica de invalidación de la sección de datos del cliente. Esta versión introduce una nueva forma de invalidar todos los datos de las secciones del cliente que evita un problema conocido con el almacenamiento local cuando se personaliza `sections.xml` las invalidaciones están activas. (Anteriormente, el contenido privado (almacenamiento local) no se rellenaba correctamente cuando tenía una variable personalizada *etc/frontend/sections.xml* con invalidaciones de acción). Consulte [Contenido privado](https://developer.adobe.com/commerce/php/development/cache/page/private-content/#invalidate-private-content).
...
* La aplicación ya no muestra un error cuando se crea correctamente un pedido y una RMA desde la tienda. Anteriormente, la aplicación creaba el RMA como se esperaba, pero también mostraba este error:  `We can't create a return right now. Please try again later`.

<!-- MC-30181-->

* Los vínculos de seguimiento incluidos en los retornos de pedidos de tienda ahora funcionan según lo esperado. Anteriormente, cuando un cliente hacía clic en el vínculo de seguimiento del envío en la página de devolución del pedido, la aplicación mostraba este error: `PHP Fatal error: Uncaught Error: Call to a member function getTitle() on null in /app/z7kvt3uys6daq/var/view_preprocessed/pub/static/vendor/magento/module-shipping/view/frontend/templates/tracking/details.phtml:1`.

<!-- MC-30237-->

* La pestaña Devuelve ahora está presente como se espera después de crear un retorno de pedido desde el administrador.
...
* La aplicación ahora devuelve los puntos de recompensa como se espera cuando se devuelve un pedido a una tienda que se ejecuta en un entorno de varias tiendas en el que se establecen diferentes tasas de puntos de recompensa para cada tienda. Anteriormente, la aplicación no reembolsaba los puntos como se esperaba para un pedido devuelto comprado mediante puntos de recompensa.
...
* Los administradores con acceso restringido a un sitio web solamente en una implementación de varios sitios ahora pueden obtener una vista previa de una actualización programada para ese sitio web. Anteriormente, la vista previa de ensayo utilizaba el almacén predeterminado de forma predeterminada y, si el usuario no tenía acceso al almacén predeterminado, la aplicación arrojaba una excepción.

<!-- MC-32736-->

* La edición de una programación existente ya no genera una programación duplicada. Anteriormente, cuando editaba una programación existente, la aplicación la duplicaba y, cuando intentaba abrir la programación duplicada, la aplicación generaba un error.

<!-- MC-32432-->

* Los productos de venta cruzada ahora se muestran según lo esperado en las páginas Ver y Editar carro de compras después de crear una actualización de programa para uno o más de estos productos.
...
* La aplicación ya no genera un error al editar y guardar el `NOT LOGGED IN` grupo de clientes cuando se instala B2B.
...
* Ahora puede usar el Generador de páginas para agregar un producto como vínculo de botón (**Editar contenido** > **Vínculo de botón**). Anteriormente, la aplicación generaba este error al intentar seleccionar el producto: `Product with ID: XXXX doesn't exist`.
...
* La página editar categoría ahora se comporta como se espera cuando arrastra y suelta productos a una nueva posición. Anteriormente, al arrastrar y soltar productos en cualquier página de categoría, se colocaba la ventana de visualización en la parte superior de la página.
...
* La aplicación ahora guarda correctamente y muestra un nuevo nombre para una lista de deseos cuando cambia el nombre de una lista de deseos en implementaciones que contienen varias listas de deseos. Anteriormente, la aplicación seguía mostrando el nombre de la lista de deseos anterior.

<!-- MC-31948-->

* Ahora puede navegar y modificar las listas de deseos del carro de compras en las implementaciones que ejecutan Internet Explorer 11.x. Anteriormente, no se podía crear, editar ni mover una lista de deseos en este entorno.
...
**Problema**: Las anomalías en los mensajes de error de tienda se producen en implementaciones en las que está instalado PHP 7.4.2. Cuando se implementa 2.4.0 con PHP 7.4.2, los símbolos de espacio en los mensajes de error de tienda se sustituyen por caracteres más (+). Este error es nativo de PHP 7.4.2 y no puede ser corregido por Adobe Commerce. **Solución alternativa**: Se recomienda utilizar otras versiones de PHP 7.4.x. Consulte [Visualización de datos de mensajes sin procesar en una tienda](https://support.magento.com/hc/en-us/articles/360045804332)  Artículo de la Base de conocimiento. <!-- MC-34170-->
...
**Problema**: Los comerciantes no pueden interactuar con ningún elemento de página de la página Devuelve después de crear una etiqueta de envío para una autorización de devolución de mercancías (RMA). Consulte [Devuelve: la página Editar deja de funcionar al crear una etiqueta de envío](https://support.magento.com/hc/en-us/articles/360046441312) Artículo de la Base de conocimiento. La variable **MC-35984-2.4.0-composer.patch** la corrección de este problema ya está disponible en [Versiones](https://magento.com/tech-resources/download). <!-- MC-35984-->

**Problema**: Los administradores no pueden agregar un producto configurable por SKU a una cotización. Cuando un administrador hace clic en la variable **Agregar a cotización** , la página Edición de comillas permanecía en estado de carga y el administrador no podía guardar los cambios. **Solución alternativa**: No existe una solución alternativa para la edición de ofertas B2B.  Sin embargo, puede seguir solicitando productos seleccionando los productos de la lista de productos en lugar de agregarlos por SKU. Consulte [El administrador B2B no puede agregar un producto configurable a una cotización](https://support.magento.com/hc/en-us/articles/360046801971) Artículo de la Base de conocimiento. <!-- MC-35513-->
...
**Problema**: Si un código de descuento caduca antes de que se apruebe una orden de compra (OC), la orden de compra continúa mostrando el importe descontado, pero una vez aprobada la orden de compra, se coloca en el total sin descuento. **Solución alternativa**: Instale el **B2B-709 Parche de descuento por orden de compra** corrección para este problema, que ahora está disponible en la **Mi cuenta** > **Descargas** sección de `magento.com`.

**Problema**: La aplicación supone una excepción durante la actualización a B2B 1.2.0 en una implementación multisitio web. When `setup:upgrade` se ejecuta, este error se produce en la variable `PurchaseOrder` módulo: `Module Magento_PurchaseOrder: Unable to apply data patch Magento\PurchaseOrder\Setup\Patch\Data\InitPurchaseOrderSalesSequence for moduleMagento_PurchaseOrder`. **Solución alternativa**: Instale el **B2B-716 Añadir la interfaz NonTransactionableInterface al parche de datos InitPurchaseOrderSalesSequence** revisión, que ahora está disponible desde la **Mi cuenta** > **Descargas** sección de `magento.com`.
...
* **Problema**: Las pruebas de integración fallan con este error: `PHP Fatal error: Declaration of Dotdigitalgroup\Email\Test\Integration\Model\Sync\Importer\ImporterFailedTest::setUp() must be compatible with PHPUnit\Framework\TestCase::setUp(): void in /var/www/vendor/dotmailer/dotmailer-magento2-extension/Test/Integration/Model/Sync/Importer/ImporterFailedTest.php on line 36`. **Solución alternativa**: La variable **parche de las pruebas de integración digital BUNDLE-2684** la corrección de este problema ya está disponible en [Versiones](https://magento.com/tech-resources/download). <!-- BUNDLE-2684-->
