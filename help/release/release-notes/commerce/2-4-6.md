...
## 2.4.6-beta1 entorno de prueba

Adobe Commerce 2.4.6-beta1 se ha probado con las siguientes versiones de componentes:
...
Esta versión incluye ocho correcciones de seguridad y mejoras de seguridad de la plataforma. Estas correcciones de seguridad se han backportado a Adobe Commerce 2.4.5-p1 y Adobe Commerce 2.4.4-p3.
...
PHP 8.1 sigue siendo totalmente compatible. Adobe Commerce 2.4.6 GA incluirá soporte para PHP 8.2. Sin embargo, el retraso en la publicación de PHP 8.2 nos ha impedido incluir soporte para él en esta versión beta. <!-- AC-6598 -->

La compatibilidad con PHP 7.4 se ha eliminado de esta versión. No puede ejecutar Adobe Commerce en PHP 7.4.

Adobe Commerce 2.4.6-beta1 ahora admite:

* Compositor 2.2.x <!-- AC-2839 -->

* Versión 7.0.x. Aunque la compatibilidad con Redis 6.2 se mantiene, recomendamos implementar esta versión con Redis 7.0.x porque Redis 6.2 dejará de funcionar en abril de 2024. <!-- AC-6606 -->
...
* MariaDB 10.6 (última versión). Esta versión sigue siendo compatible con MariaDB 10.4, pero recomendamos actualizar a MariaDB 10.6. <!-- AC-6921 -->
...
La función Orden de compra para empresas ahora está totalmente expuesta por la API de GraphQL. Esta versión presenta dos esquemas de GraphQL:

* El esquema de GraphQL de orden de compra cubre la funcionalidad de Administración de órdenes de compra del usuario de la empresa B2B, como crear, editar y eliminar órdenes de compra.
...
Estamos solucionando cientos de problemas en el código principal de Adobe Commerce 2.4.6. A continuación se describe un subconjunto de los problemas corregidos incluidos en Adobe Commerce 2.4.6-beta1.
...
* En ejecución `bin/magento set:di:com -vvv` (o con el `-v` o `-vv` ) ahora devuelve un mensaje informativo en caso de error. [GitHub-35391](https://github.com/magento/magento2/issues/35391)
...
* La variable `composer install` el cuerpo ya no se sobrescribe `.gitignore` archivos. [GitHub-32888](https://github.com/magento/magento2/issues/32888)

<!-- AC-6671-->

* Ejecución `php bin/magento staticdata:remove` para eliminar datos de ejemplo después de la actualización ya no falla con este error: `Undefined constant "MagentoHackathon\Composer\Magento\Deploystrategy\GLOB_BRACE`. [GitHub-36062](https://github.com/magento/magento2/issues/36062)
...
* El recuento de elementos de la categoría ahora es preciso después de filtrar la página Catálogo cuando **Mostrar sin existencias** está activada. Anteriormente, el recuento de tiendas de productos fuera de existencias era incorrecto.
...
* Los compradores ya no pueden evitar la validación de reCAPTCHA al realizar un pedido con el Braintree cuando reCAPTCHA está habilitado.

<!-- BUNDLE-3185-->

* La validación de ReCAPTCHA durante el cierre de compra para pedidos enviados a varios destinos ahora funciona según lo esperado.
...
* El rendimiento de carga de página ha mejorado cuando se carga la lista de productos del widget de catálogo. Anteriormente, los atributos no globales se cargaban directamente desde la base de datos.
...
* Ahora se generan direcciones URL compatibles con SEO para productos cuyos `url_key` se sobrescribe en el nivel de vista de tienda.
