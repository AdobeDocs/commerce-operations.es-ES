...
**Problema**: Es posible que los elementos de infraestructura de RabbitMQ (intercambios, colas) no se inicialicen después de una nueva instalación (no actualización) de Adobe Commerce 2.4.5. Esta configuración incorrecta de RabbitMQ puede provocar un error fatal cuando se ejecutan operaciones asincrónicas o se genera un perfil de rendimiento. Las implementaciones de Cloud no se ven afectadas. **Solución alternativa**: Los comerciantes que realicen una nueva instalación de Adobe Commerce 2.4.5 local deben ejecutarse `bin/magento setup:upgrade --keep-generated` después de usar `bin/magento setup:config:set` para configurar RabbitMQ o instalar Adobe Commerce pasando las opciones de configuración de RabbitMQ a la variable `setup:install` comando. Consulte [RabbitMQ](../../../installation/prerequisites/rabbitmq.md).
