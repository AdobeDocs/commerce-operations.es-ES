---
title: 'Política de aplicación de seguridad: acciones y plazos requeridos'
...
badgePaas: label="Solo Adobe Commerce en la nube" type="Informative" url="https://experienceleague.adobe.com/es/docs/commerce/user-guides/product-solutions" tooltip="Solo se aplica a los proyectos de Adobe Commerce en la nube."
hide: true
...
# Política de aplicación de la seguridad: acciones y plazos requeridos
...
La política de seguridad corporativa de Adobe requiere que todos los entornos alojados por Adobe para Adobe Commerce en la nube se ejecuten en un software seguro y compatible.
...
1. Adobe Commerce en Cloud (versión 2.4.8, 2.4.9 o la versión más reciente)

Esto sirve para mitigar los riesgos de seguridad en los entornos de comercio electrónico. A los entornos que no cumplan estos requisitos dentro de los plazos establecidos en [Tabla 1](#determine-your-required-actions) se les suspenderá el tráfico entrante, con lo que la tienda quedará sin conexión. Tenga en cuenta este aviso como un requisito de seguridad y cumplimiento con fechas de ejecución.
...
En la tabla siguiente, busque su versión de Adobe Commerce en la nube a continuación para ver qué se necesita de usted.

**Tabla 1: Acciones requeridas y fechas límites por versión**

| **Su versión** | **[Acción 1:<br>Actualizar dependencias de software de terceros](#action-1-upgrade-third-party-software-dependencies)**&#x200B; | &#x200B;** Acción 2:<br>[Actualice o migre su versión de Adobe Commerce](#action-2-upgrade-to-a-supported-adobe-commerce-version)** |
| --- | --- | --- |
| 2.4.4 o 2.4.5 | Acción requerida para el 30 de octubre de 2026. | Acción requerida para el 1 de junio de 2027 |
| 2.4.6 o 2.4.7 | Acción requerida para el 30 de octubre de 2026 o el 31 de mayo de 2027, según el software. | Acción requerida para el 1 de junio de 2028 |
| 2.4.8 o 2.4.9 | Acción requerida para el 30 de octubre de 2026 o el 31 de mayo de 2027, según el software. | No se requiere ninguna acción en este momento |

## ¿Quién no necesita tomar medidas?

Este aviso no se aplica a:

* Clientes que usan [!DNL Adobe Commerce as a Cloud Service]
* Clientes que utilizan Adobe Commerce en la versión en la nube 2.4.8 o 2.4.9 con dependencias de software compatibles en todos los entornos

### Verificar las versiones actuales

Necesita ayuda del administrador de comercio electrónico para seguir los siguientes pasos y comprobar qué versión está ejecutando en cada uno de los entornos de Adobe Commerce en la nube.

#### Paso 1: Comprobar la versión de Adobe Commerce en la nube
...
1. Si la versión no se muestra en el Administrador, use la [herramienta de línea de comandos de Adobe Commerce](https://experienceleague.adobe.com/es/docs/commerce-operations/configuration-guide/cli/config-cli){target="_blank"} para ejecutar el comando de versión:

   ```shell
   bin/magento --version
   ```

#### Paso 2: Comprobar las versiones de dependencia del software
...
>[!WARNING]
>
>Si su entorno no cumple los requisitos de los plazos detallados en [Acciones requeridas y tabla de plazos](#determine-your-required-actions), Adobe se verá obligado a suspender el tráfico entrante al entorno afectado. La tienda de comercio electrónico no tendrá conexión y no dará servicio a los compradores.

## Qué requiere cada acción

### Acción 1: actualizar dependencias de software de terceros

Según el software, todas las dependencias de software no admitidas deben actualizarse en función de los plazos compartidos en la tabla siguiente. Puede ver sus entornos en [Cloud Console](https://console.adobecommerce.com/). Para comprobar las versiones de dependencias que se ejecutan en cada entorno, consulte [Comprobar versiones de dependencias de software](#check-software-dependency-versions). Las actualizaciones de dependencias de software se aplican a todas las versiones de Adobe Commerce en la nube de 2.4.4 a 2.4.9.
...
* **Introducción:** Abra un ticket de asistencia en el que se enumeren los entornos que necesita actualizar y las dependencias involucradas. Abra su boleto al menos 30 días antes de la fecha de aplicación para que nuestro equipo pueda programar el trabajo.
...
### Acción 2: actualizar a una versión compatible de Adobe Commerce

Si necesita actualizar la versión de Adobe Commerce en la nube, tiene dos opciones:

1. [Actualización a una versión compatible de Adobe Commerce en la nube](#upgrade-to-adobe-commerce-on-cloud-version-249)
1. [Migración a Adobe Commerce as a Cloud Service (plataforma SaaS)](#migrate-to-adobe-commerce-as-a-cloud-service)

La fecha de aplicación de la versión actual se aplica independientemente de la opción que elija.

**Tabla 3: Directrices y plazos para actualizar a una versión compatible de Adobe Commerce en la nube**
...
## Comparar sus opciones

Para decidir qué opción se adapta a sus necesidades, consulte la siguiente tabla comparando Adobe Commerce en la nube versión 2.4.9 con Adobe Commerce as a Cloud Service.

**Tabla 4: Adobe Commerce en la nube frente a Adobe Commerce as a Cloud Service**
...
## ¿Qué sucede si no realiza ninguna acción?

Si un entorno no ha cumplido estos requisitos en las fechas de aplicación de [Determine las acciones necesarias](#determine-your-required-actions), Adobe tomará las medidas adecuadas. Esto incluye la suspensión del tráfico a la infraestructura afectada y, como resultado, la tienda de comercio electrónico se desconectará.
...
### Actualización a Adobe Commerce en la nube versión 2.4.9
...
Adobe proporciona herramientas que reducen el coste y el tiempo de la migración a Adobe Commerce as a Cloud Service. Estas herramientas solo se aplican a la migración. No se utilizan para una actualización de versión en Adobe Commerce en la nube. Consulte la [descripción general de la migración](https://experienceleague.adobe.com/es/docs/commerce/cloud-service/migration/overview) para obtener una guía completa de la migración, que incluye las rutas y fases de la migración.
...
* **Migración de tiendas y extensiones:** Las [herramientas para desarrolladores y migración asistida por IA de Adobe](https://developer.adobe.com/commerce/extensibility/developer-agent/), entre ellas [!DNL Adobe Developer App Builder] y [!DNL Commerce Storefront powered by Edge Delivery Services], ayudan a acelerar la modernización de tiendas y la reorganización de plataformas de extensiones.
