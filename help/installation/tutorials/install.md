...
* Verifique que su sistema cumpla los requisitos descritos en la [requisitos del sistema](../system-requirements.md).
...
* Consulte la [Introducción a la instalación de la línea de comandos](../composer.md) información general.
...
>[!NOTE]
>
>De forma predeterminada, el instalador no sobrescribe la base de datos si instala el software Commerce en la misma instancia de base de datos. Puede utilizar la opción `cleanup-database` para cambiar este comportamiento.

Consulte también [Actualizar, volver a instalar, desinstalar](uninstall.md).
...
Las siguientes tablas describen los nombres y valores de las opciones de instalación, como los comandos de instalación. Consulte [Instalaciones de host local de muestra](#sample-localhost-installations).
...
En Adobe Commerce versión 2.2.8 y posteriores, puede crear el usuario administrador durante o después de la instalación. Si crea el usuario durante la instalación, se requieren todas las variables de credenciales de administrador. Consulte [Instalaciones de host local de muestra](#sample-localhost-installations).
...
| `--db-prefix` | Utilícelo solo si está instalando las tablas de base de datos en una instancia de base de datos que ya tiene tablas Adobe Commerce o Magento Open Source.<br><br>En ese caso, utilice un prefijo para identificar las tablas de esta instalación. Algunos clientes tienen más de una instancia de Adobe Commerce y Magento Open Source ejecutándose en un servidor con todas las tablas de la misma base de datos.<br><br>El prefijo puede tener un máximo de cinco caracteres de longitud. Debe comenzar con una letra y solo puede incluir letras, números y caracteres de subrayado.<br><br>Esta opción permite a esos clientes compartir el servidor de base de datos con más de una instalación. | No |
...
| `--language` | Código de idioma para usar en Admin y tienda. (Si aún no lo ha hecho, puede ver la lista de códigos de idioma introduciendo `magento info:language:list` del directorio bin.) | No |
| `--currency` | Moneda predeterminada para usar en la tienda. (Si aún no lo ha hecho, puede ver la lista de monedas introduciendo `magento info:currency:list` del directorio bin.) | No |
| `--timezone` | Zona horaria predeterminada para usar en Admin y tienda. (Si aún no lo ha hecho, puede ver la lista de zonas horarias introduciendo `magento info:timezone:list` del directorio bin.) | No |
...
| `--cleanup-database` | Para soltar tablas de base de datos antes de instalar la aplicación, especifique este parámetro sin un valor. De lo contrario, la base de datos se mantiene intacta. | No |
...
>[!TIP]
>
>Para habilitar los servicios de almacenamiento remoto durante la instalación, consulte [Configuración del almacenamiento remoto](../../configuration/remote-storage/remote-storage.md) en el _Guía de configuración_.

**Opciones de configuración del motor de búsqueda:**

| Nombre | Valor | ¿Requerido? |
|--- |--- |--- |
| `--search-engine` | Versión del motor de búsqueda. Los valores posibles son `elasticsearch7`, `elasticsearch6`y `elasticsearch5`. El valor predeterminado es `elasticsearch7`. Si ha instalado OpenSearch como motor de búsqueda, especifique el valor `elasticsearch7`. El Elasticsearch 5 ha quedado obsoleto y no se recomienda. | No |
...
| `--elasticsearch-username` | El ID de usuario para autenticarse | No, a menos que la autenticación esté habilitada |
...
|--- |--- |--- |
| `remote-storage-driver` | Nombre del adaptador<br>Valores posibles:<br>**file**: Deshabilita el almacenamiento remoto y utiliza el filesystem local <br>**aws-s3**: Utilice la variable [Amazon Simple Storage Service (Amazon S3)](https://aws.amazon.com/s3/) | No |
...
| `remote-storage-region` | Nombre de la región | No |
...
El siguiente ejemplo instala la aplicación con las siguientes opciones:
...
* La aplicación Magapplication se instala en la variable `magento2` directorio relativo al servidor web docroot en `localhost` y la ruta al administrador es `admin`; por lo tanto:
...
* Utilice un `ORD$` prefijo de incremento de pedido de ventas (ya que contiene un carácter especial) [`$`], el valor debe estar entre comillas dobles)
...
>[!TIP]
>
>Si tiene una cuenta de usuario para acceder al servidor de aplicaciones, consulte [establecer una máscara](../next-steps/set-umask.md). Este tipo de configuración es típico del alojamiento compartido.
