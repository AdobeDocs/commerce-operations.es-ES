...
|----------------|-----------------|
| `upgrade:check` | <ul><li>—ayuda: Devuelve todas las opciones disponibles.</li><li>—current-version: Versión actual de Adobe Commerce. Este parámetro es obligatorio y siempre debe usarse.</li><li>—min-issue-level: Puede filtrar los problemas según el nivel de problema mínimo (el valor predeterminado es ADVERTENCIA).</li><li>—ignore-current-version-compatibility-issues (o -i): Si no desea incluir problemas críticos, errores y advertencias de la versión actual en el informe.</li><li>—coming-version (o -c): Oriente una versión específica de Adobe Commerce. Si se omite, se utilizará la versión más reciente disponible.</li></ul> |
...
Es posible ejecutar `--help` como opción al ejecutar un comando específico. Devuelve `--help` para el comando especificado.
