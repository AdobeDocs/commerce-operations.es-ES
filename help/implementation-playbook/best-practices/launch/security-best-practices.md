...
![Lista de comprobación](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Elija un socio o integrador de soluciones (SI) que tenga en cuenta la seguridad**: garantice integraciones seguras y código personalizado mediante la selección de organizaciones con un historial de desarrollo seguro.
...
![Lista de comprobación](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Código de auditoría**: revise el servidor y el repositorio de código fuente para ver si quedan elementos de desarrollo. Asegúrese de que no existan archivos de registro accesibles, directorios .git, túneles SQL, volcados de base de datos o archivos de información php.

## Actualice a la última versión

Adobe lanza continuamente componentes de soluciones actualizados para mejorar la seguridad y proteger mejor a los clientes contra posibles riesgos. La actualización a la última versión de la aplicación de Adobe Commerce, los servicios instalados y las extensiones, así como la aplicación de los parches actuales, constituyen el principal método de protección contra las amenazas a la seguridad.
...
- [Aviso de seguridad y cumplimiento](../../../release/security-enforcement-policy.md) (Adobe Commerce en las versiones 2.4.4 a 2.4.9 de Cloud)
- [Guía de actualización](../../../upgrade/overview.md)
- [Cómo aplicar los parches](../../../upgrade/patches/overview.md)

>[!TIP]
>
>Obtenga la información de seguridad más reciente y mitigue los problemas de seguridad conocidos mediante la suscripción al [Servicio de notificación de seguridad de Adobe](https://www.adobe.com/subscription/adobesecuritynotifications.html).
...
Esta sección resume las prácticas recomendadas para mantener la seguridad del sitio y de la infraestructura en una instalación de Adobe Commerce. Muchas de estas prácticas recomendadas se centran en proteger la infraestructura informática en general, por lo que algunas de las recomendaciones ya se han aplicado.

![Lista de comprobación](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **Bloquear el acceso no autorizado**: para bloquear el acceso no autorizado al sitio de Commerce y a los datos de los clientes, trabaje con su socio de alojamiento para configurar un túnel VPN. Para bloquear el acceso no autorizado a la aplicación de Commerce, configure un túnel SSH.
...
Los ataques de malware dirigidos a sitios de comercio electrónico son frecuentes, y los actores de amenazas continuamente desarrollan nuevas formas de obtener información personal y de tarjetas de crédito a partir de transacciones.

Sin embargo, Adobe ha descubierto que la mayoría de los compromisos del sitio no se deben a un atacante innovador. Por el contrario, los agentes de amenazas suelen aprovechar las vulnerabilidades existentes sin parches, las contraseñas incorrectas y la configuración débil de propiedad y permisos en el sistema de archivos.
...
Después de acciones como el envío de formularios o la modificación de campos, el simulador serializa los datos y los envía a puntos de conexión de terceros. Estos extremos suelen ser otros sitios web comprometidos que actúan como relé para enviar los datos a su destino final.


>[!TIP]
>
>Si un ataque de malware afecta a un sitio de Commerce, sigue las prácticas recomendadas de Adobe Commerce para [responder a un incidente de seguridad](../maintenance/respond-to-security-incident.md).
...
- **Botnets**: el servidor Commerce del cliente pasa a formar parte de un botnet que envía correo electrónico no deseado. Aunque los datos de usuario no suelen estar comprometidos, los filtros de correo no deseado lista de bloqueados el nombre de dominio del cliente, lo que impide la entrega de cualquier correo electrónico desde el dominio. Alternativamente, el sitio del cliente pasa a formar parte de una botnet, lo que provoca un ataque de denegación de servicio (DDoS) distribuido en otros sitios. La botnet bloquea el tráfico IP entrante al servidor de Commerce, lo que impide que los clientes puedan realizar compras.
...
- Identifique y proteja todos los puntos desde donde se pueda acceder a la instalación de Commerce desde el mundo exterior.
  ...
  Puede proteger el acceso al administrador, que requiere la mayor protección, si sigue las [recomendaciones de prioridad](#priority-recommendations) de Adobe al configurar su proyecto de Commerce.
